...
v4.4.0 keeps playback identity separate from sport identity.  Every transport/provider
...
class PlaybackReadinessStore:
    ...
    def record(self, event, session):
        ...
        with self._lock, closing(self._connect()) as conn:
            ...
            row = dict(existing) if existing else {
                ...
                "recovered_failovers": 0, "consecutive_failures": 0, "startup_samples_json": "[]",
            ...
            if event == "selection":
                ...
                warm_failures += 1; score -= 5; last_error = _clean(session.get("lastError") or session.get("reason"), 700)
            elif event == "failure":
            ...
            next_row = {
                **row,
                ...
                "consecutive_failures": consecutive, "startup_samples_json": json.dumps(samples, separators=(",", ":")),
            ...
            conn.execute("""
                INSERT INTO playback_asset_health(
                  ...
                  startup_samples_json,first_seen_at,last_seen_at,last_success_at,last_failure_at,quarantined_until,last_error,updated_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                ON CONFLICT(media_key) DO UPDATE SET
                  ...
                  consecutive_failures=excluded.consecutive_failures,startup_samples_json=excluded.startup_samples_json,
                  last_seen_at=excluded.last_seen_at,last_success_at=excluded.last_success_at,last_failure_at=excluded.last_failure_at,
            ...
            """, (
                ...
                next_row["startup_samples_json"], float(row.get("first_seen_at") or now), now, last_success, last_failure,
    ...
    def summary(self):
        ...
        with closing(self._connect()) as conn:
            ...
            recent = conn.execute("SELECT media_key,competition_id,provider,transport,state,reliability_score,last_error,updated_at FROM playback_asset_health ORDER BY updated_at DESC LIMIT 12").fetchall()
        return {
            ...
            "recent": [dict(r) for r in recent], "generatedAt": now,
        }
...
def _install_hook_once():
    ...
    with _HOOK_LOCK:
        if getattr(MilestoneConsole, "_sbb_playback_readiness_wrapped", False):
            return True
        original = getattr(MilestoneConsole, "record_playback", None)
        if not callable(original):
            return False

        def wrapped(self, event, session, *args, **kwargs):
            result = original(self, event, session, *args, **kwargs)
            try: default_store().record(event, session or {})
            except Exception: pass
            return result

        MilestoneConsole.record_playback = wrapped
