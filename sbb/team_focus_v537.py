"""Sports Big Board v5.4.4 — participant metadata + Team Focus enrichment.
...
def _http_text(url, timeout=6.0):
    req = Request(url, headers={"User-Agent": "SportsBigBoard/5.4.4 (+team-focus-cache)", "Accept": "text/html,application/json;q=0.9,*/*;q=0.8"})
...
def _install_into_server():
    ...
    if not (isinstance(loaded, dict) and isinstance(loaded.get("leagues"), dict)):
        ...
        # the background into the v5.4.4 cache.
