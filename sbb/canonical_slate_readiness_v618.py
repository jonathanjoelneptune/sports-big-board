"""Sports Big Board v6.1.8 canonical slate readiness repair.
...
VERSION = "6.1.8-canonical-slate-readiness-1"
...
def _install_connection_tuning():
    ...
    if getattr(cls, "__sbbV618WritableConnectionTuning", False):
        return
    ...
    cls.__sbbV618WritableConnectionTuning = True


def _install_ncaaf_state_precedence():
    cls = shadow.CanonicalShadowStore
    if getattr(cls, "__sbbV618NcaafStatePrecedence", False):
        return
    ...
    cls.__sbbV618NcaafStatePrecedence = True
...
def _install_canonical_date_comparisons():
    cls = shadow.CanonicalShadowStore
    if getattr(cls, "__sbbV618CanonicalDateComparisons", False):
        return
    ...
    cls.__sbbV618CanonicalDateComparisons = True
...
def _repair_ncaaf_policy_states(store):
    ...
    for event in events:
        ...
        by_class = {}
        for row in rows:
            source_class = _clean(row[0]).upper()
            by_class.setdefault(source_class, row)
        if not {"AUTHORITATIVE", "INDEPENDENT"} <= set(by_class):
            continue

        states = []
        for source_class in ("AUTHORITATIVE", "INDEPENDENT"):
            raw = _json_load(by_class[source_class][1])
...
def _install_engine_observability(engine):
    ...
    if getattr(cls, "__sbbV618ReadinessObservability", False):
        return
    original_run = cls.run_horizon
    original_health = cls.health

    def run_horizon(self):
        self.__sbbV618RunStartedAt = time.time()
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                result["wallSecondsV618"] = round(time.perf_counter() - started, 3)
                self.last_stats = result
            return result
        finally:
            self.__sbbV618RunStartedAt = 0.0

    def health(self):
        payload = original_health(self)
        started_at = float(getattr(self, "__sbbV618RunStartedAt", 0.0) or 0.0)
    ...
    cls.health = health
    cls.__sbbV618ReadinessObservability = True
...
def install():
    ...
    threading.Thread(target=_runtime_repair, daemon=True, name="sbb-canonical-slate-readiness-v618").start()
