...
for token in ['aggregateReason(','sbb-daily-recap-context','/api/league-view?league=','WILD CARD / PLAYOFF RACE','AP TOP 25','BIG BOARD EVENT VIEW','BRACKET / ROUNDS']:
    assert token in league+league_css,token
for token in ['ESPN_COMPETITIONS','/api/league-view','playoffRace','rankings','specialEvent','league-view-v538.json']:
...
for token in ['sbb-browse-entity-abbr','installLegacyCfbGuard()','enterSpecialContext(','auditDate(row)<=todayLocal','shortEntityName(teams[0])} vs ${shortEntityName(teams[1])',"contextInsight('RESULT',`${compactDate(row.date)} · ${row.label} · ${row.result} ${row.score}`"]:
...
for token in ['SBB_EARLY_PAUSE_RECOVERY','USER_PAUSE_SUPPRESS','SOFT_RESUME','BOUNDED_RECOVERY','5200','7900']:
...
print('PASS v5.3.8 League View + recap identity + team history cutoff + accessible theming + special-event context + early-pause recovery')
