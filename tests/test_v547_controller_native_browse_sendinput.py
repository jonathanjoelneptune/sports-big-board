#!/usr/bin/env python3
"""v5.4.8 controller-native Team/Player Browse and SendInput fullscreen bridge."""
...
assert 'browseApi()?.controllerEntityEntries?.(league)' in core
assert 'api?.controllerBrowseEntity' in core
assert "return playerBrowseLeague(league)?'PLAYER BROWSE':'TEAM BROWSE';" in core
assert 'async function controllerEntityEntriesForLeague(league)' in browse
...
print('PASS v5.4.8 controller-native team/player browse + SendInput fullscreen bridge acknowledgement')
