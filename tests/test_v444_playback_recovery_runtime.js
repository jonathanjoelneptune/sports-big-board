...
(async()=>{
  ...
  let s=T.endurance.snapshot();assert.equal(s.retryAttempts,1);assert.equal(restoreCalls,1);assert.equal(resetCalls,1);assert.equal(s.status,'RUNNING');
  ...
  console.log('PASS: v4.4.4 clean retry recovery + no-repeat media guard');
