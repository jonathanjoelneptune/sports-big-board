...
css=(ROOT/'ui'/'browse-curated-programming-v534.css').read_text()
...
for surface in [
    f'ui/browse-curated-programming-v534.css?v={version}',
    f'ui/workspace-viewport-fit-v531.css?v={version}',
    f'<script src="ui/browse-curated-programming-v534.js?v={version}"></script>',
...
for token in [
    "const VERSION='5.3.4'",'placeBrowseControls()','positionPopover()','primeEntityCatalog()',
    "subnav.innerHTML='<button id=\"sbbBrowseBtn\"", "browseWord=state.entityType==='player'?'PLAYER BROWSE':'TEAM BROWSE'",
...
for token in ["const VERSION='5.3.4'","body.classList.contains('sbb-game-center-side')",'viewportHeight()-top-bottomGap',"sbb:browse-layout",'SBB_WORKSPACE_VIEWPORT_FIT']:
...
print('PASS v5.3.4 contextual subnav Browse, date-headed scored timeline, automatic chronological queue, and open/closed viewport fit')
