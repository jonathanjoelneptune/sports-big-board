...
assert(!hist.includes('scoreCardPlayableItems(match)'),'historical on-demand click may not re-enter the synchronous legacy score resolver');
assert(hist.includes('const started=await playGameHighlights'),'historical media recovery delegates exhaustion/unavailable state to the same cooperative v5 score-click transaction');
