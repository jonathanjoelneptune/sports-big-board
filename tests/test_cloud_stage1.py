...
class CloudStage1Tests(unittest.TestCase):
    def test_frontend_loads_api_runtime_before_bootstrap(self):
        ...
        self.assertIn('config.js?v=4.1.10',html); self.assertIn('api-runtime.js?v=4.1.10',html)
        self.assertLess(html.index('api-runtime.js?v=4.1.10'),html.index('BOOT_START'))
    ...
    def test_version_file_matches_server(self):
        self.assertEqual((ROOT/'VERSION').read_text().strip(),'4.1.10')
        self.assertIn('APP_VERSION = "4.1.10"',(ROOT/'server.py').read_text())
