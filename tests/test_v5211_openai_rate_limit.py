...
checks={
    'ticker backend version': f'{VERSION}-sports-ticker-4' in backend,
...
if failed:
    print(f'FAIL v{VERSION} OpenAI Sports Ticker rate-limit invariants')
    for name in failed: print(' -',name)
    raise SystemExit(1)
print(f'PASS v{VERSION} OpenAI Sports Ticker rate-limit invariants')
