...
for token in [
    ...
    "v5.3.13 same-game curated fallback",
    "v5.3.13 next special-event highlight after unavailable source",
...
assert 'if(special){body+=roundsCard(payload.games||[],true);body+=gamesCard(payload.games||[],true);body+=localEventCard(context);}' in league
...
assert 'v5.3.13 — standings-first hierarchy' in league_css
...
print('PASS v5.3.13 special-event playback isolation + Game Center freshness + division/Wild Card League View')
