...
The Silver tab has dedicated filters for scope, league, kind, integrity flag, period, and text search. Its summary shows collections, asset links, unique assets, daily/weekly counts, suspicious links, and the largest collection. CSV/XLSX export follows the active audit tab, so Silver can be exported separately from game media. The audit is read-only and does not alter collection membership or game playback truth.
