...
This patch is intentionally idempotent. The v6.1.16 release materializer replays
historical Media Audit generations first, then re-applies this lane so production
receives the same behavior as the checked-in source tree.
"""
...
def patch_service() -> bool:
    ...
    store_anchor = "    def record_repair_source_attempt(self, repair_id, event_key, stage, *, provider='', query='', result_count=0,"
...
def patch_ui() -> bool:
    ...
    new = "${fmtNum(rs.knownTransportRefreshes||0)} transport refreshes • ${fmtNum(rs.salvageTransportPersisted||0)} persisted • ${fmtNum(rs.salvageCandidatesSelected||0)} salvage selected • ${fmtNum(rs.salvageCertified||0)} salvage certified • ${fmtNum(rs.salvageClosedHealthy||0)} salvage closed • ${fmtNum(rs.youtubeIndexedVideos||0)} YT indexed"
    if new in text:
        return False
    if old not in text:
        raise SystemExit("ERROR: Media Audit repair telemetry anchor missing")
...
def patch_materializer() -> bool:
    ...
    if '"tools/patch_media_repair_salvage_v6116.py"' not in text:
        if preserve_anchor not in text:
            raise SystemExit("ERROR: v6.1.16 PRESERVE anchor missing")
    ...
    if call not in text:
        ...
        if anchor not in text:
            raise SystemExit("ERROR: v6.1.16 post-base Media Audit anchor missing")
...
def patch_verify() -> bool:
    ...
    original = text
    additions=[]
    if 'tests/test_media_repair_salvage_v6116.py' not in text:
        additions.append('python3 tests/test_media_repair_salvage_v6116.py')
    if 'tools/patch_media_repair_salvage_v6116.py' not in text:
        additions.append('python3 -m py_compile tools/patch_media_repair_salvage_v6116.py')
    if additions:
