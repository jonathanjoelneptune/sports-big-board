#!/usr/bin/env python3
"""Atomic checkout materializer for Sports Big Board v6.1.0.

The GitHub web repository intentionally retains many v5.5-era active source files;
the deployment workflow materializes the release generation inside each clean
Actions checkout. v6.1 must therefore collapse BOTH 5.5.0 and 6.0.0 active tokens
to 6.1.0 before verification/deployment/Pages build.

This helper also installs the v6.1 certification service, extends VERIFY.sh,
adds Canonical Slate/health buttons to the production frontend, and materializes
the controller navigation document expected by the release-integrity checker.
"""
...
def patch_init(root: Path, dry=False):
    ...
    if "_install_canonical_certification_v610()" in text:
        return False
    block = (
        "\n\n# v6.1.0: canonical authoritative + independent schedule certification.\n"
        "# Shadow-only: certification changes canonical evidence state, never production authority.\n"
...
def patch_verify(root: Path, dry=False):
    ...
    changed = False
    additions = []
    if "tests/test_v610_canonical_certification.py" not in text:
        additions.append("python3 tests/test_v610_canonical_certification.py")
    if "sbb/canonical_certification_v610.py" not in text:
        additions.append("python3 -m py_compile sbb/canonical_certification_v610.py")
    if additions:
        changed = True
...
def materialize_controller_map(root: Path, dry=False):
    ...
    sources = [root / "CONTROLLER-REGION-MAP-v6.0.0.md", root / "CONTROLLER-REGION-MAP-v5.5.0.md"]
    source = next((x for x in sources if x.is_file()), None)
    if source is None:
        if target.is_file():
            return False
        raise SystemExit("ERROR: no controller-region-map source is available for v6.1 materialization")
...
def main(argv=None):
    ...
    if current not in set(OLD_VALUES) | {NEW}:
        raise SystemExit(f"ERROR: expected source release 5.5.0/6.0.0/6.1.0, found {current!r}")
    required = [
    ...
    if missing:
        raise SystemExit("ERROR: v6.1 web overlay incomplete: " + ", ".join(missing))
    ...
    print(f"Sports Big Board v6.1 release materialization -> {NEW}")
    ...
    if checker.is_file():
        ...
        if result.returncode:
            print("ERROR: release-integrity checker failed after v6.1 materialization", file=sys.stderr)
            return result.returncode
        print("PASS: release identity is synchronized at 6.1.0")
    else:
