...
# v5.4.2 Premium Masthead + Sports Ticker + Score Ribbon. This remains a
...
if f'ui/premium-masthead-v5214.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 premium masthead stylesheet')
for required in [
...
# v5.4.2 Premium Now Watching Experience. This is a presentation-only late
...
if f'ui/premium-now-watching-v5215.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 Premium Now Watching stylesheet')
for required in [
...
# v5.4.2 Editorial Slugs + Integrated Up Next. The visual queue prefers the
...
if f'ui/editorial-slugs-up-next-v5216.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 editorial/up-next stylesheet')
if f'<script src="ui/up-next-experience-v5217.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 integrated Up Next module')
for required in [
    ...
    if required not in upnext_css:
        errors.append(f'v5.4.2 visual contract missing: {required}')
for required in [
    ...
    if required not in upnext_js:
        errors.append(f'v5.4.2 Up Next behavior contract missing: {required}')
for forbidden in ['setInterval(', 'requestAnimationFrame(loop', 'new MutationObserver']:
    if forbidden in upnext_js:
        errors.append(f'v5.4.2 Up Next module adds continuous observation/work: {forbidden}')
try:
    ...
    if not (watching_pos2 < upnext_css_pos < index.index('</head>')):
        errors.append('v5.4.2 visual stylesheet load order is unsafe')
    if not (app_pos2 < ticker_pos2 < upnext_js_pos):
        errors.append('v5.4.2 Up Next module must load after app and ticker ownership')
except ValueError:
    errors.append('index is missing v5.4.2 Up Next release surfaces')


# v5.4.2 Drawer Polish + Harmonized Controls. These are late presentation and
...
if f'ui/harmonized-controls-drawer-v5217.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 harmonized-controls stylesheet')
if f'<script src="ui/harmonized-controls-drawer-v5217.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 drawer-polish module')
for required in [
    ...
    if required not in polish_css:
        errors.append(f'v5.4.2 harmonized-controls visual contract missing: {required}')
for required in [
    ...
    if required not in polish_js:
        errors.append(f'v5.4.2 drawer-polish behavior contract missing: {required}')
try:
    ...
    if not (old_css_pos < polish_css_pos < index.index('</head>')):
        errors.append('v5.4.2 harmonized-controls stylesheet load order is unsafe')
    if not (upnext_js_pos2 < polish_js_pos):
        errors.append('v5.4.2 drawer polish must load after integrated Up Next')
except ValueError:
    errors.append('index is missing v5.4.2 drawer-polish release surfaces')


# v5.4.2 Game Center Workspace Reflow. The final late layer owns the actual
...
if f'ui/viewing-workspace-v5218.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 viewing-workspace stylesheet')
if f'<script src="ui/viewing-workspace-v5218.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 viewing-workspace module')
for required in [
    ...
    if required not in workspace_css:
        errors.append(f'v5.4.2 viewing-workspace visual contract missing: {required}')
for required in [
    ...
    if required not in workspace_js:
        errors.append(f'v5.4.2 viewing-workspace behavior contract missing: {required}')
try:
    ...
    if not (old_polish_css < workspace_css_pos < index.index('</head>')):
        errors.append('v5.4.2 viewing-workspace stylesheet must load after v5.2.17 polish')
    if not (old_polish_js < workspace_js_pos):
        errors.append('v5.4.2 viewing-workspace module must load after v5.2.17 polish')
except ValueError:
    errors.append('index is missing v5.4.2 viewing-workspace release surfaces')


# v5.4.2 Game Center Readability + Full Content Scroll. The provider renderer
...
if f'ui/game-center-readability-v5219.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 Game Center readability stylesheet')
if f'<script src="ui/game-center-readability-v5219.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 Game Center readability module')
for required in [
    ...
    if required not in gc_read_css:
        errors.append(f'v5.4.2 Game Center readability visual contract missing: {required}')
for required in [
    ...
    if required not in gc_read_js:
        errors.append(f'v5.4.2 Game Center readability behavior contract missing: {required}')
for forbidden in ['setInterval(', 'requestAnimationFrame(loop']:
    if forbidden in gc_read_js:
        errors.append(f'v5.4.2 Game Center readability adds continuous work: {forbidden}')
try:
    ...
    if not (workspace_css_pos2 < read_css_pos < index.index('</head>')):
        errors.append('v5.4.2 Game Center readability stylesheet must load after workspace reflow')
    if not (workspace_js_pos2 < read_js_pos):
        errors.append('v5.4.2 Game Center readability module must load after workspace reflow')
except ValueError:
    errors.append('index is missing v5.4.2 Game Center readability release surfaces')

# v5.4.2 explicit Game Center scroll owner + score-interrupt queue projection.
...
if f'ui/game-center-scroll-v5220.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 Game Center scroll stylesheet')
if f'<script src="architecture/score-interrupt-queue-v5220.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 score-interrupt queue module')
if f'<script src="ui/game-center-scroll-v5220.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 Game Center scroll module')
for required in [
    ...
    if required not in gc_scroll_css:
        errors.append(f'v5.4.2 Game Center hard-scroll visual contract missing: {required}')
for required in [
    ...
    if required not in gc_scroll_js:
        errors.append(f'v5.4.2 Game Center hard-scroll behavior contract missing: {required}')
for required in [
    ...
    if required not in interrupt_js:
        errors.append(f'v5.4.2 score-interrupt queue contract missing: {required}')
for required in [
    ...
    if required not in upnext_js:
        errors.append(f'v5.4.2 Up Next interrupt projection contract missing: {required}')
for forbidden in ['setInterval(', 'requestAnimationFrame(loop', 'new MutationObserver']:
    if forbidden in interrupt_js or forbidden in gc_scroll_js:
        errors.append(f'v5.4.2 scroll/interrupt module adds continuous work: {forbidden}')
try:
    ...
    if not (app_pos3 < interrupt_pos < upnext_pos3):
        errors.append('v5.4.2 score-interrupt queue must load after app and before Up Next')
    if not (read_css_pos2 < scroll_css_pos < index.index('</head>')):
        errors.append('v5.4.2 hard-scroll stylesheet must load after Game Center readability')
    if not (read_js_pos2 < scroll_js_pos):
        errors.append('v5.4.2 hard-scroll module must load after Game Center readability')
except ValueError:
    errors.append('index is missing v5.4.2 scroll/interrupt release surfaces')


# v5.4.2 Clean Collapse + Viewport Fit. Collapsing the information drawer must
...
if f'ui/collapse-viewport-fit-v5221.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 collapse/viewport stylesheet')
if f'<script src="ui/collapse-viewport-fit-v5221.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 collapse/viewport module')
for required in ['grid-template-columns:minmax(0,1fr) 0!important','max-width:0!important','top:50%!important','--sbb-collapsed-stage-height','aspect-ratio:auto!important']:
    if required not in collapse_css:
        errors.append(f'v5.4.2 clean-collapse visual contract missing: {required}')
for required in [f"const VERSION='{version}'",'stage.getBoundingClientRect().top','viewportHeight()-top-bottomGap',"body.style.setProperty('--sbb-collapsed-stage-height'",'SBB_COLLAPSE_VIEWPORT_FIT']:
    if required not in collapse_js:
        errors.append(f'v5.4.2 viewport-fit behavior contract missing: {required}')
try:
    scroll_css_pos=index.index(f'ui/game-center-scroll-v5220.css?v={version}')
    ...
    if not (scroll_css_pos < collapse_css_pos < index.index('</head>')):
        errors.append('v5.4.2 collapse stylesheet must load after Game Center hard-scroll layer')
    if not (scroll_js_pos < collapse_js_pos):
        errors.append('v5.4.2 collapse module must load after Game Center hard-scroll module')
except ValueError:
    errors.append('index is missing v5.4.2 collapse/viewport release surfaces')


# v5.4.2 Browse + Curated Programming. This is a user-facing discovery layer over
...
if f'ui/browse-curated-programming-v537.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 Browse stylesheet')
if f'<script src="ui/browse-curated-programming-v537.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 Browse module')
for required in [
    ...
    if required not in browse_css:
        errors.append(f'v5.4.2 Browse visual contract missing: {required}')
for required in [
    "const VERSION='5.4.2'", 'SBB_CURATED_BROWSE',
    ...
    if required not in browse_js:
        errors.append(f'v5.4.2 Browse behavior contract missing: {required}')
for forbidden in ['setInterval(', 'requestAnimationFrame(loop', 'data-curation-select', '+ QUEUE', '‹ DAY', 'id="sbbBrowseExit"']:
    if forbidden in browse_js:
        errors.append(f'v5.4.2 Browse module contains forbidden work/legacy queue affordance: {forbidden}')
for required in [
    ...
    if required not in browse_css:
        errors.append(f'v5.4.2 persistent Browse/context visual contract missing: {required}')
for required in [
    ...
    if required not in browse_js:
        errors.append(f'v5.4.2 persistent Browse/context behavior contract missing: {required}')
try:
    ...
    if not (collapse_css_pos2 < browse_css_pos < index.index('</head>')):
        errors.append('v5.4.2 Browse stylesheet must load after the v5.2 viewing polish stack')
    if not (collapse_js_pos2 < browse_js_pos):
        errors.append('v5.4.2 Browse module must load after score/game-center/viewing ownership modules')
except ValueError:
    errors.append('index is missing v5.4.2 Browse release surfaces')

# v5.4.2 viewport-fit applies to the player whether Game Center is open or closed.
...
if f'ui/workspace-viewport-fit-v531.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 workspace viewport-fit stylesheet')
if f'<script src="ui/workspace-viewport-fit-v531.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 workspace viewport-fit module')
for required in ['--sbb-workspace-stage-height','body.sbb-game-center-side .stage-card>.stage{','aspect-ratio:auto!important']:
    if required not in workspace_fit_css:
        errors.append(f'v5.4.2 workspace viewport-fit visual contract missing: {required}')
for required in [f"const VERSION='{version}'","body.classList.contains('sbb-game-center-side')",'viewportHeight()-top-bottomGap',"sbb:browse-layout",'SBB_WORKSPACE_VIEWPORT_FIT']:
    if required not in workspace_fit_js:
        errors.append(f'v5.4.2 workspace viewport-fit behavior contract missing: {required}')
try:
    ...
    errors.append('index is missing v5.4.2 Browse/viewport-fit release surfaces')

# v5.4.2 persistent participant index + TeamRankings/ESPN Team Focus enrichment.
...
for required in [
    ...
    if required not in team_focus:
        errors.append(f'v5.4.2 Team Focus backend contract missing: {required}')
if 'from .team_focus_v537 import install as _install_team_focus_v537' not in sbb_init or '_install_team_focus_v537()' not in sbb_init:
    errors.append('sbb package does not install v5.4.2 Team Focus backend')

# v5.4.2 Focus Integration + Full Team Theme.
for required in [
    ...
    if required not in source: errors.append(f'v5.4.2 focus integration contract missing: {required}')
for required in ['entities', '_espn_directory', 'POWER RANK', 'browse-participants-v538.json']:
    if required not in team_focus: errors.append(f'v5.4.2 participant/team-focus backend contract missing: {required}')
...
for required in ['shouldPreserveCurrentQueue()', 'date-owned score selection']:
    if required not in score_interrupt: errors.append(f'v5.4.2 date-owned score queue contract missing: {required}')


# v5.4.2 League View, accessible team palette, event-context navigation, and
...
for required in [
    ...
    if required not in index: errors.append(f'v5.4.2 League View release surface missing: {required}')
for required in [
    ...
    if required not in league_js: errors.append(f'v5.4.2 League View behavior missing: {required}')
for required in ['.league-view-root{','.league-view-table{','.sbb-legacy-up-next-hidden{display:none!important}','body.sbb-daily-recap-context #gameCenterContent{display:none!important}']:
    if required not in league_css: errors.append(f'v5.4.2 League View visual contract missing: {required}')
for required in ['SBB_EARLY_PAUSE_RECOVERY','5200','8200','USER_PAUSE_SUPPRESS','SOFT_RESUME','BOUNDED_RECOVERY']:
    if required not in early_pause: errors.append(f'v5.4.2 early-pause recovery missing: {required}')
for forbidden in ['setInterval(', 'requestAnimationFrame(loop']:
    if forbidden in early_pause: errors.append(f'v5.4.2 early-pause recovery contains unbounded work: {forbidden}')
for required in ['/api/league-view','ESPN_COMPETITIONS','playoffRace','conferences','leaders','rankings','specialEvent','league-view-v538.json']:
    if required not in league_backend: errors.append(f'v5.4.2 League View backend contract missing: {required}')
if 'from .league_view_v538 import install as _install_league_view_v538' not in sbb_init or '_install_league_view_v538()' not in sbb_init:
    errors.append('sbb package does not install v5.4.2 League View backend')
for required in [
    '_build_accessible_theme', '_relative_luminance', '_contrast', 'team-theme-v538.json',
    '"surfaceRaised"', '"blackReplacement"', '"wcag"'
]:
    if required not in team_focus: errors.append(f'v5.4.2 accessible team theme backend missing: {required}')
for required in [
    ...
    if required not in browse_js: errors.append(f'v5.4.2 Browse hardening contract missing: {required}')
for required in ['--sbb-team-black-replacement','--sbb-team-gradient-start','#sbbFocusPlayAll{','#sbbFocusExit{']:
    if required not in browse_css: errors.append(f'v5.4.2 theme polish contract missing: {required}')

# v5.4.2 Team Context + Drawer Sync. Team selection auto-starts the newest playable
...
for required in [
    ...
    if required not in browse_js: errors.append(f'v5.4.2 team-context behavior missing: {required}')
for forbidden in ['sbb-browse-entity-abbr\"', "class='sbb-browse-entity-abbr'", 'userPauseUntil']:
    ...
    if forbidden in source: errors.append(f'v5.4.2 retired UI/pause contract still present: {forbidden}')
for required in [
    ...
    if required not in browse_css: errors.append(f'v5.4.2 team-context visual contract missing: {required}')
for required in [
    '#infoDrawer .drawer-pane.hidden', 'body.sbb-game-center-side #infoDrawer .drawer-pane.hidden',
    'display:none!important'
]:
    if required not in league_css: errors.append(f'v5.4.2 drawer exclusivity contract missing: {required}')
for required in [
    ...
    if required not in league_js: errors.append(f'v5.4.2 active-league sync contract missing: {required}')
for required in [
    ...
    if required not in early_pause: errors.append(f'v5.4.2 persistent user-pause contract missing: {required}')
if 'tests/test_v539_team_context_drawer_sync.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 Team Context + Drawer Sync regression')

# v5.4.2 continuity guard replaces the legacy startup watchdog before it installs.
...
for required in [
    ...
    if required not in index: errors.append(f'v5.4.2 progress-watchdog surface missing: {required}')
for required in ['SBB_PLAYBACK_PROGRESS_WATCHDOG','positive startup stall','recovery-suppressed','provider reports playing; recovery suppressed']:
    if required not in progress_guard: errors.append(f'v5.4.2 continuity guard missing: {required}')
if index.index('playback-progress-watchdog-v5310.js') > index.index('playback-progress-watchdog.js'):
    errors.append('v5.4.2 continuity guard must load before the legacy progress watchdog')
if 'tests/test_v5310_special_event_playback_league_view.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 regression')

# v5.4.2 playback-context boundary + readable standings. Curated queues must
...
for required in [
    ...
    if required not in browse_js: errors.append(f'v5.4.2 curated queue release contract missing: {required}')
for required in [
    ...
    if required not in league_js: errors.append(f'v5.4.2 playback/navigation League View contract missing: {required}')
if 'state.contextPoll=setInterval' in league_js:
    errors.append('v5.4.2 League View must not use the old 1.2s polling refresh loop')
for required in [
    ...
    if required not in league_css: errors.append(f'v5.4.2 readable standings visual contract missing: {required}')
for required in [
    ...
    if required not in league_backend: errors.append(f'v5.4.2 standings backend contract missing: {required}')
if 'tests/test_v5311_playback_context_league_view.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 playback-context + League View regression')

# v5.4.2 closes the remaining Special Event stale-provider race and exposes
...
for required in [
    ...
    if required not in browse_js: errors.append(f'v5.4.2 Special Event/navigation contract missing: {required}')
for required in [
    'function specialEventBoard(', 'function groupStandings(', 'function bracketBoard(',
    'function formMarkup(form=[])'
]:
    if required not in league_js: errors.append(f'v5.4.2 tournament/form League View contract missing: {required}')
for required in ['league-view-event-group-grid','league-view-bracket-grid','league-view-form i.win','.league-view-nhl{gap:5px!important}']:
    if required not in league_css: errors.append(f'v5.4.2 tournament/form visual contract missing: {required}')
if 'tests/test_v5314_league_navigation_event_brackets.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 league navigation + Special Event regression')

# v5.4.2 preserves score-ribbon league/day semantics without doing a dense
...
for required in [
    ...
    if required not in score_interrupt:
        errors.append(f'v5.4.2 non-blocking score-ribbon league-day queue contract missing: {required}')
for forbidden in ['programForScoreDate(date)', 'scoreRibbonLeagueFilter=league;', 'setInterval(']:
    if forbidden in score_interrupt:
        errors.append(f'v5.4.2 score-ribbon queue must not use blocking/looping contract: {forbidden}')
if 'tests/test_v5316_score_ribbon_league_day_queue.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 score-ribbon league-day compatibility regression')

# v5.4.2 visually groups the normal-league TODAY / ALL / TEAM BROWSE controls
...
for required in [
    ...
    if required not in browse_css: errors.append(f'v5.4.2 league subnav visual contract missing: {required}')
for required in [
    ...
    if required not in match_center_js: errors.append(f'v5.4.2 sport Match Center contract missing: {required}')
if 'body.sbb-sport-match-center-active:not(.sbb-special-event-match-center)' not in match_center_css:
    errors.append('v5.4.2 sport Match Center visual isolation contract missing')
if 'tests/test_v5317_queue_performance_match_center.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 queue performance + Match Center regression')


# v5.4.2 makes Play All intentionally conservative: only completed games with
...
for required in [
    ...
    'v5.4.2 Play All fast same-game fallback', 'v5.4.2 Play All skipped slow verified source',
]:
    if required not in browse_js: errors.append(f'v5.4.2 verified Play All / fast-start contract missing: {required}')
for required in [
    ...
    if required not in browse_css: errors.append(f'v5.4.2 compact league subnav contract missing: {required}')
if 'tests/test_v5318_verified_play_all_fast_start.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 verified Play All + fast-start regression')


# v5.4.2 Controller foundation. The readiness module remains the single semantic
...
controller_map=text(Path('CONTROLLER-REGION-MAP-v5.4.2.md'))
if f'<script src="architecture/controller-readiness-v540.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 controller-readiness module')
if f'ui/controller-readiness-v540.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 controller-readiness stylesheet')
for required in [
    ...
    if required not in controller: errors.append(f'v5.4.2 controller-readiness contract missing: {required}')
for region in [
    ...
    if f"name:'{region}'" not in controller: errors.append(f'v5.4.2 controller region missing: {region}')
for forbidden in ['navigator.getGamepads','gamepadconnected','gamepaddisconnected','attributes:true','attributeFilter:']:
    if forbidden in controller: errors.append(f'v5.4.2 readiness layer contains premature/unsafe controller observer work: {forbidden}')
for required in ['[data-sbb-controller-focus="1"]','--sbb-focus-accent','outline:2px solid','prefers-reduced-motion']:
    if required not in controller_css: errors.append(f'v5.4.2 focus visual contract missing: {required}')
for forbidden in ['animation:','backdrop-filter','filter:blur(']:
    if forbidden in controller_css: errors.append(f'v5.4.2 controller focus CSS contains performance-risk effect: {forbidden}')
for region in ['league-nav','score-ribbon','player-transport','game-center','league-view','settings','coming-up','team-browse','history-audit']:
    if f'`{region}`' not in controller_map: errors.append(f'v5.4.2 controller region map missing documentation: {region}')
if 'tests/test_v540_controller_readiness.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 controller-readiness regression')
if 'node --check architecture/controller-readiness-v540.js' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not syntax-check v5.4.2 controller-readiness module')
try:
    ...
    if not operator_pos < controller_pos:
        errors.append('v5.4.2 controller-readiness module must load after existing UI/operator modules')
except ValueError:
    errors.append('index is missing v5.4.2 controller-readiness load-order surfaces')

# v5.4.2 Controller Radials + Live Input. The v5.4.0 readiness layer remains the
# semantic authority while v5.4.2 owns browser Gamepad discovery, robust raw-input
...
if f'<script src="architecture/controller-mode-v542.js?v={version}"></script>' not in index:
    errors.append('index is missing synchronized v5.4.2 controller module')
if f'ui/controller-mode-v542.css?v={version}' not in index:
    errors.append('index is missing synchronized v5.4.2 controller stylesheet')
for required in [
    ...
    if required not in controller_core: errors.append(f'v5.4.2 controller contract missing: {required}')
for forbidden in ['setInterval(', 'new MutationObserver']:
    if forbidden in controller_core: errors.append(f'v5.4.2 controller contains performance-risk loop/observer: {forbidden}')
for required in ['id="controllerModeSelect"','<option value="automatic">Automatic</option>','<option value="disabled">Disabled</option>','id="controllerStatusValue"','id="controllerLiveIndicator"']:
    if required not in index: errors.append(f'v5.4.2 controller surface missing: {required}')
for required in ['.sbb-controller-help','[data-sbb-controller-focus="1"]','.controller-live-indicator','.sbb-controller-radial','.sbb-controller-pointer','prefers-reduced-motion']:
    if required not in controller_mode_css: errors.append(f'v5.4.2 controller presentation contract missing: {required}')
...
if 'tests/test_v542_controller_radials_live_input.py' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not run v5.4.2 controller regression')
if 'node --check architecture/controller-mode-v542.js' not in text(Path('VERIFY.sh')):
    errors.append('VERIFY.sh does not syntax-check v5.4.2 controller module')
try:
    ...
    if not readiness_pos < core_pos: errors.append('v5.4.2 controller must load after semantic readiness')
except ValueError:
    errors.append('index is missing v5.4.2 controller load-order surfaces')
