...
required=['styles.css','config.js','api-runtime.js','core-model.js','architecture/playback-session.js','architecture/milestone-console.js','architecture/site-soundtrack.js','app.js']
...
if "version:String(DOMAIN_MODEL?.version||" not in app: errors.append('app architecture version is not derived from core model')
history_ui=(root/'ui'/'history-audit.js').read_text(encoding='utf-8')
if "const FRONTEND_VERSION=String(window.SBB_CORE?.version||" not in history_ui: errors.append('history audit frontend version is not derived from core model')
