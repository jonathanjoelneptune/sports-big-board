...
def run_base(root):
    ...
    subprocess.run([sys.executable, str(root / "tools" / "apply_v615_release.py"), "--skip-check"], cwd=root, check=True)
...
ROLLING_AUDIT_INTERVAL_SECONDS = max(30.0, float(os.environ.get("SBB_MEDIA_AUDIT_ROLLING_INTERVAL_SECONDS", "120")))
...
'''
    if "REPAIR_WORKER_COUNT = " not in text:
        text = text.replace(config_marker, config_marker + config_block, 1)
    ...
    text = text.replace('AUDIT_GENERATION = "R20-PLAYBACK-EVIDENCE-CORROBORATION"', 'AUDIT_GENERATION = "R21-CONTINUOUS-ROLLING-REPAIR"', 1)

    old_sig = '    def start_run(self, mode="ALL", start_date=""):\n'
    new_sig = '    def start_run(self, mode="ALL", start_date="", max_games=0):\n'
    text = replace_once(text, old_sig, new_sig, "bounded rolling start_run")
    loop_old = '            for event in events:\n                if mode != "ALL":\n'
    loop_new = '            for event in events:\n                if max_games and ordinal >= int(max_games):\n                    break\n                if mode != "ALL":\n'
    text = replace_once(text, loop_old, loop_new, "rolling batch size")
    ...
    backlog_method = '''    def rolling_backlog_count(self, cutoff_date, fresh_seconds=FRESH_SECONDS):
        ...
        with closing(self.connect(timeout=5)) as conn:
            rows = conn.execute(
                """SELECT e.canonical_event_key,e.event_date,e.event_json,e.final_at,p.certified_at
                   FROM history_catalog_event e
                   LEFT JOIN history_media_canonical_package p ON p.canonical_event_key=e.canonical_event_key
                   WHERE e.event_date<=? AND (p.canonical_event_key IS NULL OR p.certified_at<?)""",
                (cutoff_date, stale_before),
            ).fetchall()
        count = 0
        for row in rows:
            event = _jloads(row["event_json"], {})
            if _event_final(str(row["event_date"] or ""), float(row["final_at"] or 0), event):
                count += 1
        return count
...
"""
    ...
    success_old = "            return {\"ok\":bool(payload.get('ok',True)),\"payload\":payload,\"reason\":str(payload.get('error') or '')}\n"
    success_new = "            REPAIR_DISCOVERY_CIRCUIT.success()\n            return {\"ok\":bool(payload.get('ok',True)),\"payload\":payload,\"reason\":str(payload.get('error') or '')}\n"
    text = replace_once(text, success_old, success_new, "repair discovery circuit success")
    http_old = "            return {\"ok\":False,\"reason\":str(payload.get('error') or f'HTTP_{exc.code}')}\n"
    http_new = """            reason=str(payload.get('error') or f'HTTP_{exc.code}')
            if exc.code == 429 or exc.code >= 500:
                REPAIR_DISCOVERY_CIRCUIT.failure(reason)
            return {"ok":False,"reason":reason,"infra":bool(exc.code == 429 or exc.code >= 500)}
"""
    text = replace_once(text, http_old, http_new, "repair discovery HTTP failures")
    exc_old = "        except Exception as exc:\n            return {\"ok\":False,\"reason\":f\"DISCOVERY_TRANSPORT_{type(exc).__name__.upper()}\",\"infra\":True,\"message\":str(exc)}\n"
    exc_new = """        except Exception as exc:
            reason=f"DISCOVERY_TRANSPORT_{type(exc).__name__.upper()}"
            REPAIR_DISCOVERY_CIRCUIT.failure(reason)
            return {"ok":False,"reason":reason,"infra":True,"message":str(exc)}
"""
    text = replace_once(text, exc_old, exc_new, "repair discovery transport failures")
    ...
    globals_old = "REPAIR_ENGINE = None\n\ndef _spawn_workers(reason=\"service start\"):\n"
    globals_new = "REPAIR_ENGINE = None\nREPAIR_ENGINES = []\nROLLING_COORDINATOR = None\n\ndef _spawn_workers(reason=\"service start\"):\n"
    text = replace_once(text, globals_old, globals_new, "background service globals")
    ...
    with WORKER_CONTROL_LOCK:
        ...
        if existing and str(existing.get("state") or "").upper() in {"RUNNING", "PAUSED"}:
            return existing
        run = STORE.start_run("STALE", cutoff_date, max_games=batch_size)
...
'''
    ...
    startup_old = '''REPAIR_ENGINE = MediaRepairEngine(STORE, DB_WRITER)
REPAIR_ENGINE.start()
'''
    startup_new = '''REPAIR_ENGINES = [
...
'''
    text = replace_once(text, startup_old, startup_new, "multi-repair + rolling startup")

    status_old = '                    "repair": {**(cached.get("repairSummary") or {}), "worker": REPAIR_ENGINE.snapshot() if REPAIR_ENGINE else {}},\n                    "run": cached.get("run"), "summary": cached.get("summary") or {},\n'
    status_new = '                    "repair": {**(cached.get("repairSummary") or {}), "worker": REPAIR_ENGINE.snapshot() if REPAIR_ENGINE else {}, "workers": [w.snapshot() for w in REPAIR_ENGINES], "workerCount": len(REPAIR_ENGINES), "discoveryCircuit": REPAIR_DISCOVERY_CIRCUIT.snapshot()},\n                    "rolling": ROLLING_COORDINATOR.snapshot() if ROLLING_COORDINATOR else {},\n                    "run": cached.get("run"), "summary": cached.get("summary") or {},\n'
    text = replace_once(text, status_old, status_new, "status repair/rolling telemetry")

    stop_old = '        if REPAIR_ENGINE: REPAIR_ENGINE.stop()\n        DB_WRITER.stop(); STATUS_CACHE.stop(); threading.Thread(target=server.shutdown, daemon=True).start()\n'
    stop_new = '        if ROLLING_COORDINATOR: ROLLING_COORDINATOR.stop()\n        for _engine in REPAIR_ENGINES: _engine.stop()\n        DB_WRITER.stop(); STATUS_CACHE.stop(); threading.Thread(target=server.shutdown, daemon=True).start()\n'
    text = replace_once(text, stop_old, stop_new, "shutdown background services")
    finally_old = '        if REPAIR_ENGINE: REPAIR_ENGINE.stop()\n        DB_WRITER.stop(); STATUS_CACHE.stop(); server.server_close()\n'
    finally_new = '        if ROLLING_COORDINATOR: ROLLING_COORDINATOR.stop()\n        for _engine in REPAIR_ENGINES: _engine.stop()\n        DB_WRITER.stop(); STATUS_CACHE.stop(); server.server_close()\n'
    text = replace_once(text, finally_old, finally_new, "close background services")
...
def main(argv=None):
    ...
    if args.dry_run:
        print("v6.1.6: rolling yesterday-first audit batches + two repair workers + discovery circuit breaker")
        return 0
    run_base(root)
    ...
    print("Media Audit: rolling cutoff=today-1d batch=250 auditWorkers=1")
