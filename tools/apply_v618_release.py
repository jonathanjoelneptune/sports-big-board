#!/usr/bin/env python3
"""Sports Big Board v6.1.8 canonical slate-readiness materializer."""
...
def run_base(root):
    ...
    import apply_v617_release
    rc = apply_v617_release.main(["--skip-check"])
    if rc not in (None, 0):
        raise SystemExit(rc)
...
def patch_install_chain(root):
    ...
    if hardening_call not in text or validation_call not in text:
        raise SystemExit("ERROR: canonical hardening/validation chain missing after v6.1.7 materialization")
    if readiness_call not in text:
        block = (
            "\n\n# v6.1.8: canonical slate readiness repair.\n"
            "# Shadow-only: storage throughput, NCAAF policy precedence, and canonical ET date ownership.\n"
            "from .canonical_slate_readiness_v618 import install as _install_canonical_slate_readiness_v618\n"
            "_install_canonical_slate_readiness_v618()"
        )
        text = text.replace(validation_call, validation_call + block, 1)
        path.write_text(text, encoding="utf-8")
    text = path.read_text(encoding="utf-8")
    if not (text.index(hardening_call) < text.index(validation_call) < text.index(readiness_call)):
        raise SystemExit("ERROR: canonical install order must be hardening -> validation -> readiness")
...
def patch_verify(root):
    ...
    commands = (
        "python3 tests/test_v618_canonical_slate_readiness.py",
        "python3 -m py_compile sbb/canonical_slate_readiness_v618.py",
    )
...
def main(argv=None):
    ...
    required = [
        root / "tools" / "apply_v617_release.py",
        root / "sbb" / "canonical_slate_readiness_v618.py",
        root / "tests" / "test_v618_canonical_slate_readiness.py",
    ]
    missing = [str(x.relative_to(root)) for x in required if not x.is_file()]
    if missing:
        raise SystemExit("ERROR: incomplete v6.1.8 release: " + ", ".join(missing))
    if args.dry_run:
        print("v6.1.8: preserve v6.1.7 Media Audit and install canonical readiness after validation")
        return 0
    run_base(root)
    ...
    promote(root)
    controller(root)
    print("Sports Big Board v6.1.8 materialized")
    ...
    if tools not in sys.path:
        sys.path.insert(0, tools)
    import check_release_version  # noqa: F401 - importing executes the release checker
    return 0
