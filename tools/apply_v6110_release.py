#!/usr/bin/env python3
"""Sports Big Board v6.1.10 canonical run-integrity materializer."""
...
def run_base(root):
    ...
    import apply_v619_release
    rc = apply_v619_release.main(["--skip-check"])
    if rc not in (None, 0):
        raise SystemExit(rc)
...
def patch_install_chain(root):
    ...
    for required in (hardening_call, validation_call, readiness_call, identity_call):
        if required not in text:
            raise SystemExit(f"ERROR: canonical install-chain prerequisite missing: {required}")
    if integrity_call not in text:
        block = (
            "\n\n# v6.1.10: canonical run integrity + source-window persistence batching.\n"
            "# Shadow-only: active-event certification, transactional diagnostics, staged refresh observability.\n"
            "from .canonical_run_integrity_v6110 import install as _install_canonical_run_integrity_v6110\n"
            "_install_canonical_run_integrity_v6110()"
        )
        text = text.replace(identity_call, identity_call + block, 1)
        path.write_text(text, encoding="utf-8")
    text = path.read_text(encoding="utf-8")
    if not (
        text.index(hardening_call) < text.index(validation_call)
        < text.index(readiness_call) < text.index(identity_call) < text.index(integrity_call)
    ):
        raise SystemExit(
            "ERROR: canonical install order must be hardening -> validation -> readiness -> identity/ingestion -> run integrity"
        )
...
def patch_verify(root):
    ...
    commands = (
        "python3 tests/test_v6110_canonical_run_integrity.py",
        "python3 -m py_compile sbb/canonical_run_integrity_v6110.py",
    )
...
def main(argv=None):
    ...
    required = [
        root / "tools" / "apply_v619_release.py",
        root / "sbb" / "canonical_run_integrity_v6110.py",
        root / "tests" / "test_v6110_canonical_run_integrity.py",
    ]
    missing = [str(x.relative_to(root)) for x in required if not x.is_file()]
    if missing:
        raise SystemExit("ERROR: incomplete v6.1.10 release: " + ", ".join(missing))
    if args.dry_run:
        print("v6.1.10: preserve v6.1.9 + parallel media/UI work and install canonical run-integrity repair")
        return 0
    run_base(root)
    ...
    promote(root)
    controller(root)
    print("Sports Big Board v6.1.10 materialized")
    print("Canonical run integrity: active-event validation + atomic source batching + staged refresh observability")
    if args.skip_check:
        return 0
    tools = str(root / "tools")
    if tools not in sys.path:
