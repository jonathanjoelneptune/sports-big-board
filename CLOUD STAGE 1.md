# Sports Big Board v4.1.30 — Cloud Stage 1

v4.1.30 keeps the existing always-on Stage 1 deployment while replacing the historical media catalog with the normalized v4 baseline. Application releases remain immutable; the persistent disk is reconstructed separately and only after a passing audit.


## v4.1.30 soundtrack storage
...
After deploying v4.1.30, perform the one-time upload described in `SOUNDTRACK SETUP.md`. For the current project:
...
## v4.1.30 one-push GitHub deployment
...
[deploy] Backend v4.1.30-... is healthy.
```
...
2. Upload `sports-big-board-v4.1.30.zip` to Cloud Shell and extract it.
...
```bash
cd ~/storage/downloads/sports-big-board-v4.1.30/sports-big-board-v4.1.30
...
### v4.1.30 catalog-preserving preflight
