# Sports Big Board v4.2.0 — Cloud Stage 1

v4.2.0 is the Milestone 1 Playback & Platform Reliability release. It keeps the existing always-on Stage 1 architecture and normalized v4 catalog, adds release-generation enforcement and milestone observability, and does not require a catalog rebuild. Application releases remain immutable; persistent state remains on the existing data disk.


## v4.2.0 soundtrack storage
...
After deploying v4.2.0, perform the one-time upload described in `SOUNDTRACK SETUP.md`. For the current project:
...
## v4.2.0 one-push GitHub deployment
...
[deploy] Backend v4.2.0-... is healthy.
```
...
2. Upload `sports-big-board-v4.2.0.zip` to Cloud Shell and extract it.
...
```bash
cd ~/storage/downloads/sports-big-board-v4.2.0/sports-big-board-v4.2.0
...
### v4.2.0 catalog-preserving preflight
