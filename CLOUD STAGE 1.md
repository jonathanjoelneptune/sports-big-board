# Sports Big Board v4.1.29 — Cloud Stage 1

v4.1.29 keeps the existing always-on Stage 1 deployment while replacing the historical media catalog with the normalized v4 baseline. Application releases remain immutable; the persistent disk is reconstructed separately and only after a passing audit.


## v4.1.29 soundtrack storage
...
After deploying v4.1.29, perform the one-time upload described in `SOUNDTRACK SETUP.md`. For the current project:
...
## v4.1.29 one-push GitHub deployment
...
[deploy] Backend v4.1.29-... is healthy.
```
...
2. Upload `sports-big-board-v4.1.29.zip` to Cloud Shell and extract it.
...
```bash
cd ~/storage/downloads/sports-big-board-v4.1.29/sports-big-board-v4.1.29
...
### v4.1.29 catalog-preserving preflight
