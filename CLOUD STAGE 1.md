# Sports Big Board v4.1.27 — Cloud Stage 1

v4.1.27 keeps the existing always-on Stage 1 deployment while replacing the historical media catalog with the normalized v4 baseline. Application releases remain immutable; the persistent disk is reconstructed separately and only after a passing audit.


## v4.1.27 soundtrack storage

The soundtrack is intentionally outside both the GitHub repository and the Compute Engine persistent state disk. GitHub Pages serves `assets/soundtrack/manifest.json`; the browser streams MP3s directly from a Cloud Storage bucket named `<GCP_PROJECT_ID>-soundtrack`. This keeps normal repository clones and Pages artifacts small and prevents soundtrack traffic from competing with the Sports Big Board API/media proxy.

After deploying v4.1.27, perform the one-time upload described in `SOUNDTRACK SETUP.md`. For the current project:
...
The Pages build automatically derives `https://storage.googleapis.com/sportsbigboard-soundtrack` from `GCP_PROJECT_ID`. An optional `SBB_SOUNDTRACK_BASE_URL` repository variable can override the audio origin later if the library moves behind a CDN.

## v4.1.27 one-push GitHub deployment
...
[deploy] Backend v4.1.27-... is healthy.
```
...
2. Upload `sports-big-board-v4.1.27.zip` to Cloud Shell and extract it.
...
```bash
cd ~/storage/downloads/sports-big-board-v4.1.27/sports-big-board-v4.1.27
...
### v4.1.27 catalog-preserving preflight
