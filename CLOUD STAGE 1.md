# Sports Big Board v4.1.20 — Cloud Stage 1

v4.1.20 keeps the existing always-on Stage 1 deployment while replacing the historical media catalog with the normalized v4 baseline. Application releases remain immutable; the persistent disk is reconstructed separately and only after a passing audit.

## v4.1.20 one-push GitHub deployment
...
[deploy] Backend v4.1.20-... is healthy.
```
...
2. Upload `sports-big-board-v4.1.20.zip` to Cloud Shell and extract it.
...
### v4.1.20 catalog-preserving preflight
