# Sports Big Board v4.2.1 — Cloud Stage 1

v4.2.1 is the Milestone 1 Playback & Platform Reliability release. It keeps the existing always-on Stage 1 architecture and normalized v4 catalog, adds release-generation enforcement and milestone observability, and does not require a catalog rebuild. Application releases remain immutable; persistent state remains on the existing data disk.



## v4.2.1 milestone hardening

v4.2.1 keeps the v4.2 backend-first deployment and post-Pages production handshake, including bounded retries for transient public-endpoint stalls. Runtime startup now limits the first schedule-sync pass to the near-term horizon and defers the full schedule sweep. Operator diagnostics use staggered cached reads so the health surfaces do not themselves become a continuous database workload. After deployment, allow the backend to settle briefly, run the Milestone Release Console stress test, and compare the exported log against the v4.2.0 baseline.

## v4.2.1 soundtrack storage
...
After deploying v4.2.1, perform the one-time upload described in `SOUNDTRACK SETUP.md`. For the current project:
...
## v4.2.1 one-push GitHub deployment
...
[deploy] Backend v4.2.1-... is healthy.
```
...
2. Upload `sports-big-board-v4.2.1.zip` to Cloud Shell and extract it.
...
```bash
cd ~/storage/downloads/sports-big-board-v4.2.1/sports-big-board-v4.2.1
...
### v4.2.1 catalog-preserving preflight
