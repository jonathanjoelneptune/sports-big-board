/* Sports Big Board v5.3.4 — Browse UX + Curated Timeline
...
(() => {
  'use strict';
  if(window.SBB_CURATED_BROWSE?.version==='5.3.4') return;
  ...
  const state={
    ...
    suggestionRows:[],lastQuery:'',entityCatalogCache:new Map(),entityCatalogInflight:new Map(),
  ...
  function programItemFromAudit(row,media,tier,index=0){
    ...
    return {
      ...
      verifiedPlayable:true,verified:true,__sbbCuratedOverride:true,__sbbBrowseV534:true,
    };
  }
  ...
  function gameFromResident(match){
    try{
      ...
      const queueTitle=entityMatchupLabel(away,home);
      const items=(selection?.selectionItems||[]).filter(Boolean).map(x=>({...x,queueTitle,__sbbCuratedOverride:true,__sbbBrowseV534:true}));
      if(!items.length)return null;
  ...
  function setOpen(open){
    ...
    if(state.open){syncLeagueUi();const cached=state.entityCatalogCache.get(state.league);if(cached)renderSuggestions(cached,'');else{const host=$('sbbBrowseSuggestions');if(host)host.innerHTML=`<div class="sbb-browse-empty sbb-browse-loading">Loading all ${state.entityType==='player'?'players':'teams'} with verified highlights…</div>`;}positionPopover();primeEntityCatalog();setTimeout(()=>$('sbbBrowseSearch')?.focus(),0);}
  }
  function syncLeagueUi(){
    ...
    if(previousLeague&&previousLeague!=='ALL'&&nextLeague!==previousLeague&&state.mode!=='daily'){
      state.mode='daily';state.entity='';state.facet='';state.games=[];state.selected.clear();state.error='';
      document.body.classList.remove('sbb-curation-active');$('sbbCurationRibbon')?.classList.add('hidden');
    ...
    if(eligible&&!state.entityCatalogCache.has(state.league)&&!state.entityCatalogInflight.has(state.league))setTimeout(()=>primeEntityCatalog({render:false}),0);
  }
  ...
  async function primeEntityCatalog({render=true}={}){
    const league=state.league;if(!league||league==='ALL')return [];
    if(state.entityCatalogCache.has(league)){
      const names=state.entityCatalogCache.get(league)||[];if(render&&state.open&&state.league===league)renderSuggestions(names,$('sbbBrowseSearch')?.value||'');return names;
    }
    ...
    if(render&&state.open){const host=$('sbbBrowseSuggestions');if(host)host.innerHTML=`<div class="sbb-browse-empty sbb-browse-loading">Loading all ${state.entityType==='player'?'players':'teams'} with verified highlights…</div>`;}
    const promise=fetchFullEntityCatalog(league).then(names=>{state.entityCatalogCache.set(league,names);return names;}).finally(()=>state.entityCatalogInflight.delete(league));
    ...
    try{const names=await promise;if(render&&state.open&&state.league===league)renderSuggestions(names,$('sbbBrowseSearch')?.value||'');return names;}catch(err){
      const fallback=currentEntities(league);if(render&&state.open&&state.league===league)renderSuggestions(fallback,$('sbbBrowseSearch')?.value||'');return fallback;
    }
  }
  ...
  function activateQueue(items,label,startIndex=0){
    ...
    if(typeof tuneProgramIndexV5==='function'){tuneProgramIndexV5(bounded,{userInitiated:true,reason:`v5.3.4 curated programming: ${label}`});return true;}
    return false;
  }
  ...
  function patchRenderQueue(){
    try{
      if(typeof renderQueue!=='function'||renderQueue.__sbbBrowseV534)return false;const original=renderQueue;
      ...
      wrapped.__sbbBrowseV534=true;wrapped.__sbbOriginal=original;renderQueue=wrapped;try{window.renderQueue=wrapped;}catch(_){}state.renderQueuePatched=true;return true;
