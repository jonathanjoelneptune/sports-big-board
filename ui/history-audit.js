...
(() => {
  ...
  function consoleFullReport(data){
    ...
    const gateway=data?.youtubeGateway||{}, budget=data?.youtubeSearchBudget||{}, hi=data?.highlightly||{}, focus=data?.focus||{}, assoc=data?.associations||{}, pool=data?.greenPool||{}, providers=data?.providerConcurrency||{}, claims=data?.eventClaims||[], eff=data?.discoveryEfficiency||{}, silver=data?.silver||{}, objectives=data?.mediaObjectives||{}, objRuntime=objectives?.runtime||{}, catchup=data?.officialSourceCatchup||{}, ruleGame=data?.ruleGameCatchup||{}, ruleCollections=data?.ruleCollectionCatchup||{}, dbAudit=data?.databaseAudit||{}, silverIdentity=data?.silverIdentity||{};
  ...
  function renderConsole(data){
    ...
    const search=(data.youtubeGateway||{}).search||{}; const budget=data.youtubeSearchBudget||{}; const assoc=data.associations||{}; const eff=data.discoveryEfficiency||{}; const silver=data.silver||{}; const objectives=data.mediaObjectives||{}; const objRuntime=objectives.runtime||{}; const catchup=data.officialSourceCatchup||{}; const ruleGame=data.ruleGameCatchup||{}; const ruleCollections=data.ruleCollectionCatchup||{}; const dbAudit=data.databaseAudit||{}; const silverIdentity=data.silverIdentity||{};
    const problems=[...(data.problems||[])];
