/* Sports Big Board v4.1.16 historical database audit view. */
(() => {
  const $ = id => document.getElementById(id);
  const FRONTEND_VERSION='4.1.16';
  ...
  function consoleFullReport(data){
    ...
    const rows=[
      ...
      `[EPL PLAYLIST VIDEOS] PL hydrated ${Number((objRuntime.eplPlaylistTelemetry||{}).plClubHydrated||0)} • event matches ${Number((objRuntime.eplPlaylistTelemetry||{}).plClubEventMatches||0)} | NBC hydrated ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcHydrated||0)} • highlight titles ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcHighlightTitles||0)} • event matches ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcEventMatches||0)} | Every Goal hydrated ${Number((objRuntime.eplPlaylistTelemetry||{}).everyGoalHydrated||0)} • matchweek videos ${Number((objRuntime.eplPlaylistTelemetry||{}).everyGoalMatches||0)}`,
  ...
  async function loadConsole(){
    ...
    try{
      ...
      if(!r.ok||!data.ok){
        ...
        const out=$('historySearchConsoleOutput');if(out)out.textContent=`[ERROR] ${msg}\n\nOpen /api/status or check GitHub Actions backend deployment. The v4.1.16 workflow now refuses to publish Pages unless the public backend reports the same release version.`;
        return;
      }
      renderConsole(data);
    }catch(err){
