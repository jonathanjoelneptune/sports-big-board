/* Sports Big Board v4.1.18 historical database audit view. */
(() => {
  const $ = id => document.getElementById(id);
  const FRONTEND_VERSION='4.1.18';
  ...
  function renderWorkerGrid(data){
    ...
    for(const name of names){
      ...
      const detail=[]; if(st.current)detail.push(st.current); if(st.ruleAffinity)detail.push(`affinity ${st.ruleAffinity}${st.ruleAffinityStrict?' STRICT':(st.ruleAffinityComplete?' COMPLETE':'')}`); if(st.workType)detail.push(st.workType); if(st.catchupSources)detail.push(`sources ${st.catchupSources}`); if(st.provider)detail.push(`provider ${st.provider}`); if(st.claimKey)detail.push(`lease ${st.claimKey}`); if(st.lastDurationSeconds)detail.push(`last ${Number(st.lastDurationSeconds).toFixed(1)}s`); if(Number(st.shortCircuits||0))detail.push(`short ${Number(st.shortCircuits||0)}`); if(Number(st.fallbackAttempts||0))detail.push(`fallback ${Number(st.fallbackHits||0)}/${Number(st.fallbackAttempts||0)}`); if(!detail.length)detail.push(`heartbeat ${ageText(st.heartbeatAgeSeconds)}`);
  ...
  function consoleFullReport(data){
    ...
    const rows=[
      ...
      `[EPL PLAYLIST VIDEOS] PL hydrated ${Number((objRuntime.eplPlaylistTelemetry||{}).plClubHydrated||0)} • event matches ${Number((objRuntime.eplPlaylistTelemetry||{}).plClubEventMatches||0)} | NBC hydrated ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcHydrated||0)} • highlight titles ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcHighlightTitles||0)} • event matches ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcEventMatches||0)} | Every Goal ids ${Number((objRuntime.eplPlaylistTelemetry||{}).everyGoalVideoIds||0)} • details ${Number((objRuntime.eplPlaylistTelemetry||{}).everyGoalVideoDetails||0)} • hydrated ${Number((objRuntime.eplPlaylistTelemetry||{}).everyGoalHydrated||0)} • matchweek videos ${Number((objRuntime.eplPlaylistTelemetry||{}).everyGoalMatches||0)}`,
      `[EPL NBC MATCHING] titles ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcTitlesExamined||0)} • teams parsed ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcTeamsParsed||0)} • dates parsed ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcDatesParsed||0)} • pair ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcPairMatches||0)} • duration ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcDurationPass||0)} • associated ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcAssociationPass||0)} • persisted ${Number(objRuntime.eplNbcPlaylistExtendedPersisted||0)} • parse/team/date/duration/assoc misses ${Number((objRuntime.eplPlaylistTelemetry||{}).nbcParseFailures||0)}/${Number((objRuntime.eplPlaylistTelemetry||{}).nbcTeamMisses||0)}/${Number((objRuntime.eplPlaylistTelemetry||{}).nbcDateMisses||0)}/${Number((objRuntime.eplPlaylistTelemetry||{}).nbcDurationMisses||0)}/${Number((objRuntime.eplPlaylistTelemetry||{}).nbcAssociationMisses||0)}`,
      `[EPL PL CLUB MATCHING] titles ${Number((objRuntime.eplPlaylistTelemetry||{}).plTitlesExamined||0)} • teams parsed ${Number((objRuntime.eplPlaylistTelemetry||{}).plTeamsParsed||0)} • dates parsed ${Number((objRuntime.eplPlaylistTelemetry||{}).plDatesParsed||0)} • pair ${Number((objRuntime.eplPlaylistTelemetry||{}).plPairMatches||0)} • quick-duration ${Number((objRuntime.eplPlaylistTelemetry||{}).plQuickDurationPass||0)} • extended-duration ${Number((objRuntime.eplPlaylistTelemetry||{}).plExtendedDurationPass||0)} • associated ${Number((objRuntime.eplPlaylistTelemetry||{}).plAssociationPass||0)} • persisted Q/X ${Number(objRuntime.eplPlPlaylistQuickPersisted||0)}/${Number(objRuntime.eplPlPlaylistExtendedPersisted||0)} • parse/team/date/assoc misses ${Number((objRuntime.eplPlaylistTelemetry||{}).plParseFailures||0)}/${Number((objRuntime.eplPlaylistTelemetry||{}).plTeamMisses||0)}/${Number((objRuntime.eplPlaylistTelemetry||{}).plDateMisses||0)}/${Number((objRuntime.eplPlaylistTelemetry||{}).plAssociationMisses||0)}`,
  ...
  async function loadConsole(){
    ...
    try{
      ...
      if(!r.ok||!data.ok){
        ...
        const out=$('historySearchConsoleOutput');if(out)out.textContent=`[ERROR] ${msg}\n\nOpen /api/status or check GitHub Actions backend deployment. The v4.1.18 workflow now refuses to publish Pages unless the public backend reports the same release version.`;
        return;
      }
      renderConsole(data);
    }catch(err){
