/* Sports Big Board catalog-first runtime + interactive source management. Release identity comes from SBB_CORE. */
(() => {
  const $ = id => document.getElementById(id);
  const FRONTEND_VERSION=String(window.SBB_CORE?.version||'UNKNOWN');
  ...
  async function loadConsole(){
    ...
    try{
      ...
      if(!r.ok||!data.ok){
        ...
        const out=$('historySearchConsoleOutput');if(out)out.textContent=`[ERROR] ${msg}\n\nOpen /api/status or check GitHub Actions backend deployment. The deployment workflow refuses to publish Pages unless the public backend reports the same release version.`;
        return;
      }
      renderConsole(data);
    }catch(err){
