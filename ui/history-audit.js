/* Sports Big Board v4.1.17 historical database audit view. */
(() => {
  const $ = id => document.getElementById(id);
  const FRONTEND_VERSION='4.1.17';
  ...
  function consoleFullReport(data){
    ...
    const rows=[
      ...
      `[EPL EVENT MATCHING] titles ${Number((objRuntime.eplPlaylistTelemetry||{}).eventTitlesExamined||0)} • teams parsed ${Number((objRuntime.eplPlaylistTelemetry||{}).eventTeamsParsed||0)} • dates parsed ${Number((objRuntime.eplPlaylistTelemetry||{}).eventDatesParsed||0)} • event matches ${Number((objRuntime.eplPlaylistTelemetry||{}).eventMatches||0)} • team misses ${Number((objRuntime.eplPlaylistTelemetry||{}).eventTeamMisses||0)} • date misses ${Number((objRuntime.eplPlaylistTelemetry||{}).eventDateMisses||0)} • parse failures ${Number((objRuntime.eplPlaylistTelemetry||{}).eventParseFailures||0)}`,
  ...
  async function loadConsole(){
    ...
    try{
      ...
      if(!r.ok||!data.ok){
        ...
        const out=$('historySearchConsoleOutput');if(out)out.textContent=`[ERROR] ${msg}\n\nOpen /api/status or check GitHub Actions backend deployment. The v4.1.17 workflow now refuses to publish Pages unless the public backend reports the same release version.`;
        return;
      }
      renderConsole(data);
    }catch(err){
