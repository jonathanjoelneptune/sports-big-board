/* Sports Big Board v5.3.5 — Persistent Browse + Context Ribbon + Event Identity
...
(() => {
  'use strict';
  if(window.SBB_CURATED_BROWSE?.version==='5.3.5') return;
  ...
  function programItemFromAudit(row,media,tier,index=0){
    ...
    return {
      ...
      verifiedPlayable:true,verified:true,__sbbCuratedOverride:true,__sbbBrowseV535:true,
    };
  }
  ...
  function gameFromResident(match){
    try{
      ...
      const queueTitle=entityMatchupLabel(away,home);
      const items=(selection?.selectionItems||[]).filter(Boolean).map(x=>({...x,queueTitle,__sbbCuratedOverride:true,__sbbBrowseV535:true}));
      if(!items.length)return null;
  ...
  function ensureUi(){
    ...
    if(!$('sbbCurationRibbon')){
      ...
      ribbon.innerHTML=`<div class="sbb-curation-toolbar"><div class="sbb-curation-actions sbb-curation-actions-left"><button id="sbbCurationPlayLeft" type="button" disabled>PLAY ALL</button><button id="sbbCurationBackLeft" type="button" title="Exit curated browse and restore the daily scoreboard">EXIT</button></div><div class="sbb-curation-context"><span id="sbbCurationKicker" aria-hidden="true"></span><strong id="sbbCurationTitle">Highlights</strong><small id="sbbCurationMeta">Newest first</small></div><div class="sbb-curation-actions sbb-curation-actions-right"><button id="sbbCurationPlay" type="button" disabled>PLAY ALL</button><button id="sbbCurationBack" type="button" title="Exit curated browse and restore the daily scoreboard">EXIT</button></div></div><div id="sbbCurationCards" class="sbb-curation-cards"></div>`;
  ...
  function setEntityTickerActive(active){
    ...
    else{if($('keyInfoState')&&state.normalTickerSnapshot?.stateText)$('keyInfoState').textContent=state.normalTickerSnapshot.stateText;state.normalTickerSnapshot=null;}
  }
  ...
  function contextNews(){
    const aliases=entityAliases(state.entity);const rows=state.normalTickerSnapshot?.rows||[];return rows.filter(text=>aliases.some(alias=>norm(text).includes(alias))).slice(0,2);
  }
  ...
  async function refreshEntityTickerInsights(){
    ...
    focus.innerHTML=contextInsight(state.entityType==='player'?'PLAYER':'TEAM',state.entity,'identity')+contextInsight('LIBRARY',`${state.games.length} verified highlight game${state.games.length===1?'':'s'}`);
    try{await Promise.all(state.games.slice(0,8).map((_,i)=>enrichGameScore(i)));}catch(_){}if(token!==state.contextInsightToken)return;
    ...
    const upcoming=upcomingFromAudit();const nextText=upcoming.map(row=>`${datePill(auditDate(row))} ${entityMatchupLabel(...(splitGame(row.game).length>=2?splitGame(row.game):['','']),state.entity)}`).join(' • ');
    const recent=results.slice(0,3).map(row=>`${datePill(row.date)} ${row.result} ${row.score}`).join(' • ');
    const news=contextNews();const pieces=[
      ...
      contextInsight('NEXT 5',nextText,'next'),
      ...news.map(text=>contextInsight('NEWS',text,'news')),
    ].filter(Boolean);
    focus.innerHTML=contextInsight(state.entityType==='player'?'PLAYER':'TEAM',state.entity,'identity')+contextInsight('LIBRARY',`${state.games.length} verified highlight game${state.games.length===1?'':'s'}`)+pieces.join('');
  }
  ...
  function renderCuration(){
    ...
    if(!active){setEntityTickerActive(false);return;}
    ...
    for(const id of ['sbbCurationPlay','sbbCurationPlayLeft']){const play=$(id);if(play){play.disabled=state.loading||!state.games.length;play.textContent='PLAY ALL';}}
  ...
  function returnToDay(){state.mode='daily';state.entity='';state.facet='';state.games=[];state.entityAuditRows=[];state.selected.clear();state.error='';document.body.classList.remove('sbb-curation-active');setEntityTickerActive(false);const ribbon=$('sbbCurationRibbon');if(ribbon){ribbon.hidden=true;ribbon.classList.add('hidden');}syncLeagueUi();captureScoreRibbonHeight();window.dispatchEvent(new CustomEvent('sbb:browse-layout',{detail:{active:false}}));}
  ...
  function syncCuratedSelectedEvent(item,{deferRepair=false}={}){
    ...
    try{window.SBB_SELECTED_EVENT?.select?.(event,{source:'browse',reason:'curated playback event identity'});}catch(_){}
  ...
  function activateQueue(items,label,startIndex=0){
    ...
    if(typeof tuneProgramIndexV5==='function'){
      tuneProgramIndexV5(bounded,{userInitiated:true,reason:`v5.3.5 curated programming: ${label}`});
  ...
  function patchRenderQueue(){
    try{
      if(typeof renderQueue!=='function'||renderQueue.__sbbBrowseV535)return false;const original=renderQueue;
      ...
      wrapped.__sbbBrowseV535=true;wrapped.__sbbOriginal=original;renderQueue=wrapped;try{window.renderQueue=wrapped;}catch(_){}state.renderQueuePatched=true;return true;
