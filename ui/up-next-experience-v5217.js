/* Sports Big Board v5.3.13 — Integrated Up Next + NEXT transport repair.
...
(() => {
  'use strict';
  if(window.SBB_UP_NEXT_EXPERIENCE?.version==='5.3.13') return;
  ...
  function tuneEntry(entry){
    ...
    try{
      ...
      if(typeof tuneProgramIndexV5==='function'){
        tuneProgramIndexV5(idx,{userInitiated:true,reason:'Coming Up card selection v5.3.13'});
        return true;
      }
    }catch(err){state.lastError=String(err?.message||err);}
    return false;
  ...
  function fallbackNext(){
    try{
      ...
      tuneProgramIndexV5(target,{userInitiated:true,reason:'manual next control v5.3.13 fallback'});
      state.nextFallbacks++;
      return true;
  ...
  function repairNextButton(){
    ...
    btn.onclick=()=>{
      ...
      try{
        ...
        if(!fallbackNext())console.error('[SBB v5.3.13] NEXT control failed',err);
      }
    };
    return true;
  }
