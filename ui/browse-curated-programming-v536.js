/* Sports Big Board v5.3.6 — Persistent Browse + Context Ribbon + Event Identity
...
(() => {
  'use strict';
  if(window.SBB_CURATED_BROWSE?.version==='5.3.6') return;
  ...
  function clearTeamTheme(){const root=document.documentElement;delete root.dataset.sbbTeamTheme;for(const key of ['--sbb-team-primary','--sbb-team-secondary','--sbb-team-accent'])root.style.removeProperty(key);}
  function applyTeamTheme(){
    ...
    const palette=state.teamFocusData?.team?.palette||{};const primary=validHex(palette.primary),secondary=validHex(palette.secondary),accent=validHex(palette.accent);
    if(!primary||!secondary){clearTeamTheme();return;}
    const root=document.documentElement;root.dataset.sbbTeamTheme='on';root.style.setProperty('--sbb-team-primary',primary);root.style.setProperty('--sbb-team-secondary',secondary);root.style.setProperty('--sbb-team-accent',accent||secondary);
  }
  function syncTeamThemeToggle(){const toggle=$('teamThemeToggle');if(toggle)toggle.checked=!!state.teamThemeEnabled;const hint=$('teamThemeHint');if(hint)hint.textContent=state.teamThemeEnabled?'Team colors follow the active Team Browse selection.':'Use the normal Sports Big Board color system.';}
  ...
  function loadEntityCatalogStore(){
    try{
      const raw=JSON.parse(localStorage.getItem(ENTITY_CATALOG_KEY)||'{}');
      ...
      for(const [league,value] of Object.entries(raw)){
        ...
        state.entityCatalogCache.set(key,names);
        state.entityCatalogSavedAt.set(key,Number(value?.savedAt||0));
      }
    }catch(_){}
  }
  function persistEntityCatalog(league,names){
    const key=clean(league).toUpperCase();if(!key||!Array.isArray(names)||!names.length)return;
    const savedAt=Date.now();state.entityCatalogCache.set(key,names);state.entityCatalogSavedAt.set(key,savedAt);
    try{
      const raw=JSON.parse(localStorage.getItem(ENTITY_CATALOG_KEY)||'{}')||{};
      raw[key]={savedAt,names:[...new Set(names.map(clean).filter(Boolean))]};
      localStorage.setItem(ENTITY_CATALOG_KEY,JSON.stringify(raw));
    }catch(_){}
  }
  ...
  function programItemFromAudit(row,media,tier,index=0){
    ...
    return {
      ...
      verifiedPlayable:true,verified:true,__sbbCuratedOverride:true,__sbbBrowseV536:true,
    };
  }
  function gameFromAuditRow(row){
    const media=bestMediaForAuditRow(row);
    const items=media.items.map((x,i)=>programItemFromAudit(row,x,media.tier,i)).filter(x=>x.youtubeId||x.mediaUrl);
    // Schedule truth and media truth are deliberately separate in Team Focus.
    // An event remains visible even when discovery has not found playable media.
    return {key:`${row.league||state.league}:${row.eventId||row.game}:${row.date||''}`,date:clean(row.date),league:clean(row.league||state.league).toUpperCase(),eventId:clean(row.eventId),game:clean(row.game),tier:items.length?media.tier:'none',items,row,source:'audit',mediaAvailable:!!items.length};
  }
  function gameFromResident(match){
    try{
      ...
      const queueTitle=entityMatchupLabel(away,home);
      const items=(selection?.selectionItems||[]).filter(Boolean).map(x=>({...x,queueTitle,__sbbCuratedOverride:true,__sbbBrowseV536:true}));
  ...
  function contextIdentity(){
    const team=state.teamFocusData?.team||{},logo=clean(team.logo),label=state.entityType==='player'?'PLAYER':'TEAM';
  ...
  async function refreshEntityTickerInsights(){
    ...
    let record='',standing='';for(const game of state.games){const rs=recordStandingFromSource(game.scoreMatch||game.match||game.row||{});if(!record&&rs.record)record=rs.record;if(!standing&&rs.standing)standing=rs.standing;if(record&&standing)break;}
    const upcoming=upcomingFromAudit().slice(0,3);const nextText=upcoming.map(row=>`${datePill(auditDate(row))} ${entityMatchupLabel(...(splitGame(row.game).length>=2?splitGame(row.game):['','']),state.entity)}`).join(' • ');
    const recent=results.slice(0,3).map(row=>`${datePill(row.date)} ${row.result} ${row.score}`).join(' • ');
    ...
    const news=contextNews();const pieces=[contextIdentity(),
      contextInsight(state.entityType==='player'?'RECENT':'RECORD',state.entityType==='player'?recent:record,'record'),
      contextInsight('STREAK',state.entityType==='player'?'':streak,'streak'),
      contextInsight('STANDING',standing,'standing'),
      contextInsight('RECENT',state.entityType==='player'?'':recent,'recent'),
      contextInsight('NEXT 3',nextText,'next'),
      ...rankings.map(row=>contextInsight(clean(row.label)||'RANK',clean(row.text), 'ranking')),
      ...news.map(text=>contextInsight('NEWS',text,'news')),
    ];
  ...
  function syncLeagueUi(){
    const nextLeague=selectedLeague(),previousLeague=state.league;
  ...
  async function fetchFullEntityCatalog(league){
    ...
    // v5.3.6 prefers the backend's persisted participant index. It is built from
    ...
    try{
      ...
      const names=Array.isArray(data?.participants)?data.participants.map(clean).filter(Boolean):[];
      if(response.ok&&data?.ok&&names.length)return [...new Set(names)].sort((a,b)=>a.localeCompare(b));
    }catch(_){}
    // Compatibility fallback for a backend that has not completed the v5.3.6
  ...
  async function primeEntityCatalog({render=true,forceRefresh=false}={}){
    ...
    const promise=fetchFullEntityCatalog(league).then(names=>{if(names.length)persistEntityCatalog(league,names);return names.length?names:cached;}).finally(()=>state.entityCatalogInflight.delete(league));
  ...
  function renderSuggestions(names=[],query=''){
    ...
    host.innerHTML=merged.map(name=>`<div class="sbb-browse-suggestion"><button class="sbb-browse-entity" type="button" data-browse-entity="${esc(name)}"><span>${esc(name)}</span><small>ALL DATES</small></button><button class="sbb-browse-star ${isFavorite(state.league,name)?'active':''}" type="button" data-browse-star="${esc(name)}" aria-label="${isFavorite(state.league,name)?'Remove':'Add'} ${esc(name)} favorite">${isFavorite(state.league,name)?'★':'☆'}</button></div>`).join('');
  }
  ...
  function cardThumb(game){const item=game.items?.[0]||{};return clean(item.thumbnail)||(item.youtubeId?`https://i.ytimg.com/vi/${item.youtubeId}/mqdefault.jpg`:'')||clean(state.teamFocusData?.team?.logo);}
  ...
  async function activateHistorical({entity='',all=false}={}){
    ...
    if(state.entity&&state.entityType!=='player')loadTeamFocusData().then(()=>{if(state.mode!=='daily'&&state.entity)scheduleEntityTickerRefresh();});
    try{
  ...
  function syncCuratedGameCenterContext(item){
    ...
    if(unsupported){
      try{window.SBB_SELECTED_EVENT?.clear?.({reason:'curated competition has no Game Center',source:'browse',force:true});}catch(_){}
      try{window.SBB_GAME_CENTER_SCROLL?.repair?.();}catch(_){}
    }else if(curated){
      syncCuratedSelectedEvent(item,{deferRepair:true});
  ...
  function activateQueue(items,label,startIndex=0){
    ...
    if(typeof tuneProgramIndexV5==='function'){
      tuneProgramIndexV5(bounded,{userInitiated:true,reason:`v5.3.6 curated programming: ${label}`});
  ...
  function patchRenderQueue(){
    try{
      if(typeof renderQueue!=='function'||renderQueue.__sbbBrowseV536)return false;const original=renderQueue;
      ...
      wrapped.__sbbBrowseV536=true;wrapped.__sbbOriginal=original;renderQueue=wrapped;try{window.renderQueue=wrapped;}catch(_){}state.renderQueuePatched=true;return true;
  ...
  function init(){loadEntityCatalogStore();if(!ensureUi())return;captureScoreRibbonHeight();syncLeagueUi();bind();patchRenderQueue();setTimeout(()=>primeEntityCatalog(),0);setTimeout(patchRenderQueue,350);setTimeout(captureScoreRibbonHeight,500);}
