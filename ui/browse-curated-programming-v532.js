/* Sports Big Board v5.3.2 — Browse UX + Curated Timeline
...
(() => {
  'use strict';
  if(window.SBB_CURATED_BROWSE?.version==='5.3.2') return;
  ...
  function programItemFromAudit(row,media,tier,index=0){
    ...
    return {
      ...
      verifiedPlayable:true,verified:true,__sbbCuratedOverride:true,__sbbBrowseV532:true,
    };
  }
  ...
  function gameFromResident(match){
    try{
      ...
      const queueTitle=entityMatchupLabel(away,home);
      const items=(selection?.selectionItems||[]).filter(Boolean).map(x=>({...x,queueTitle,__sbbCuratedOverride:true,__sbbBrowseV532:true}));
      if(!items.length)return null;
  ...
  function ensureUi(){
    ...
    if(!subnav){
      ...
      subnav.innerHTML='<button id="sbbBrowseBtn" type="button" class="sbb-browse-btn" aria-haspopup="dialog" aria-expanded="false"><span>TEAM BROWSE</span><b aria-hidden="true">⌄</b></button><button id="sbbBrowseExit" type="button" class="sbb-browse-exit hidden" aria-label="Exit team browse">×</button>';
      filters.appendChild(subnav);
    }
    const btn=$('sbbBrowseBtn'),exit=$('sbbBrowseExit');
    if(!$('sbbBrowsePopover')){
  ...
  function syncLeagueUi(){
    ...
    const subnav=$('sbbBrowseSubnav'),btn=$('sbbBrowseBtn'),exit=$('sbbBrowseExit');subnav?.classList.toggle('hidden',!eligible);
    ...
    if(exit){exit.classList.toggle('hidden',state.mode==='daily');exit.title=state.entity?`Exit ${state.entity} browse`:'Exit curated browse';exit.setAttribute('aria-label',exit.title);}
  ...
  function activateQueue(items,label,startIndex=0){
    ...
    if(typeof tuneProgramIndexV5==='function'){tuneProgramIndexV5(bounded,{userInitiated:true,reason:`v5.3.2 curated programming: ${label}`});return true;}
    return false;
  }
  ...
  function patchRenderQueue(){
    try{
      if(typeof renderQueue!=='function'||renderQueue.__sbbBrowseV532)return false;const original=renderQueue;
      ...
      wrapped.__sbbBrowseV532=true;wrapped.__sbbOriginal=original;renderQueue=wrapped;try{window.renderQueue=wrapped;}catch(_){}state.renderQueuePatched=true;return true;
