/* Sports Big Board v5.3.1 — Browse UX + Curated Timeline
...
(() => {
  'use strict';
  if(window.SBB_CURATED_BROWSE?.version==='5.3.1') return;
  ...
  const state={
    ...
    scoreDateCache:new Map(),scoreDateInflight:new Map(),scoreObserver:null,
  };
  ...
  function programItemFromAudit(row,media,tier,index=0){
    ...
    return {
      ...
      verifiedPlayable:true,verified:true,__sbbCuratedOverride:true,__sbbBrowseV531:true,
    };
  }
  ...
  function gameFromResident(match){
    try{
      const candidates=scoreCardPlayableItems(match);const selection=scoreCardPlaybackSelection(match,candidates);const items=(selection?.selectionItems||[]).filter(Boolean).map(x=>({...x,__sbbCuratedOverride:true,__sbbBrowseV531:true}));
      if(!items.length)return null;
  ...
  function ensureUi(){
    ...
    if(!btn){
      btn=document.createElement('button');btn.id='sbbBrowseBtn';btn.type='button';btn.className='sbb-browse-btn hidden';btn.setAttribute('aria-haspopup','dialog');btn.setAttribute('aria-expanded','false');btn.textContent='TEAM BROWSE ▾';filters.appendChild(btn);
    }
    let exit=$('sbbBrowseExit');
    if(!exit){exit=document.createElement('button');exit.id='sbbBrowseExit';exit.type='button';exit.className='sbb-browse-exit hidden';exit.textContent='EXIT BROWSE ×';filters.appendChild(exit);}
    if(!$('sbbBrowsePopover')){
  ...
  function placeBrowseControls(){
    const btn=$('sbbBrowseBtn'),exit=$('sbbBrowseExit'),active=activeLeagueButton();if(!btn||!exit||!active)return;
    active.insertAdjacentElement('afterend',btn);btn.insertAdjacentElement('afterend',exit);
  }
  function positionPopover(){
    if(!state.open)return;const btn=$('sbbBrowseBtn'),pop=$('sbbBrowsePopover');if(!btn||!pop)return;
    const r=btn.getBoundingClientRect(),vw=Math.max(320,window.visualViewport?.width||window.innerWidth||document.documentElement.clientWidth||320),vh=Math.max(320,window.visualViewport?.height||window.innerHeight||document.documentElement.clientHeight||320);
    const width=Math.min(440,vw-16),gap=7;let left=Math.max(8,Math.min(r.left,vw-width-8));let top=r.bottom+gap;let available=vh-top-10;
    if(available<300){top=Math.max(8,Math.min(r.top-7-520,40));available=vh-top-10;}
    pop.style.left=`${Math.round(left)}px`;pop.style.right='auto';pop.style.top=`${Math.round(top)}px`;pop.style.maxHeight=`${Math.max(280,Math.floor(available))}px`;
  }

  function setOpen(open){
    state.open=!!open;const pop=$('sbbBrowsePopover'),btn=$('sbbBrowseBtn');pop?.classList.toggle('hidden',!state.open);btn?.setAttribute('aria-expanded',state.open?'true':'false');
  ...
  function syncLeagueUi(){
    ...
    const btn=$('sbbBrowseBtn'),exit=$('sbbBrowseExit');btn?.classList.toggle('hidden',!eligible);
    if(!eligible){exit?.classList.add('hidden');setOpen(false);return;}
    placeBrowseControls();
    const label=leagueLabel(state.league),browseWord=state.entityType==='player'?'PLAYER BROWSE':'TEAM BROWSE';
    if(btn)btn.textContent=`${browseWord} ▾`;
    if(exit){exit.classList.toggle('hidden',state.mode==='daily');exit.title=state.entity?`Exit ${state.entity} browse`:'Exit curated browse';}
  ...
  function renderCuration(){
    ...
    const active=state.mode!=='daily';document.body.classList.toggle('sbb-curation-active',active);ribbon.classList.toggle('hidden',!active);syncLeagueUi();
    ...
    const selectedCount=state.selected.size;const play=$('sbbCurationPlay'),clear=$('sbbCurationClearSelection');if(play){play.disabled=state.loading||!state.games.length;play.textContent=selectedCount?`PLAY QUEUED (${selectedCount})`:'PLAY ALL';}clear?.classList.toggle('hidden',!selectedCount);
  ...
  function returnToDay(){state.mode='daily';state.entity='';state.facet='';state.games=[];state.selected.clear();state.error='';document.body.classList.remove('sbb-curation-active');$('sbbCurationRibbon')?.classList.add('hidden');syncLeagueUi();window.dispatchEvent(new CustomEvent('sbb:browse-layout',{detail:{active:false}}));}
  ...
  function activateQueue(items,label,startIndex=0){
    ...
    if(typeof tuneProgramIndexV5==='function'){tuneProgramIndexV5(bounded,{userInitiated:true,reason:`v5.3.1 curated programming: ${label}`});return true;}
    return false;
  }
  ...
  function patchRenderQueue(){
    try{
      if(typeof renderQueue!=='function'||renderQueue.__sbbBrowseV531)return false;const original=renderQueue;
      ...
      wrapped.__sbbBrowseV531=true;wrapped.__sbbOriginal=original;renderQueue=wrapped;try{window.renderQueue=wrapped;}catch(_){}state.renderQueuePatched=true;return true;
  ...
  function bind(){
    ...
    $('sbbBrowseExit')?.addEventListener('click',()=>{setOpen(false);returnToDay();});
