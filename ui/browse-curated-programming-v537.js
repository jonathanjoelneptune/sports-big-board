/* Sports Big Board v5.4.6 — Game Center Score + Transition Bumpers
...
(() => {
  'use strict';
  if(window.SBB_CURATED_BROWSE?.version==='5.4.6') return;
  ...
  // v5.4.6: browser-proven embed health persists independently from release
  ...
  const state={
    ...
    // v5.4.6: Play All is a deliberately conservative unattended queue. A
  ...
  function installLegacyCfbGuard(){
    ...
    // v5.4.6: the special-event header is display-only. Do not observe/filter its
  ...
  async function fetchFullEntityCatalog(league,{forceMetadata=false}={}){
    ...
    // v5.4.6 prefers the backend's persisted participant index. It is built from
    ...
    // Compatibility fallback for a backend that has not completed the v5.4.6
  ...
  function releaseCuratedQueue(reason='return to daily programming'){
    ...
    // v5.4.6: curated Team/Player/Special Event queues are user-owned only while
  ...
  function advancePlayAllAfterSlowStart(item,index){
    ...
    if(alt){state.queueItems[index]=alt;try{PROGRAM[index]=alt;GENERAL_PROGRAM[index]=alt;}catch(_){}return tuneCuratedIndex(index,{userInitiated:false,reason:'v5.4.6 Play All fast same-game fallback'});}
    for(let next=index+1;next<state.queueItems.length;next++){const candidate=state.queueItems[next],key=programKey(candidate);if(candidate&&(!key||!state.failedCuratedMedia.has(key)))return tuneCuratedIndex(next,{userInitiated:false,reason:'v5.4.6 Play All skipped slow verified source'});}
  ...
  function syncCuratedGameCenterContext(item){
    ...
    // v5.4.6: Special Event navigation is the side-panel authority, even if an
    ...
    // Compatibility marker for pre-v5.4.6 static gates: const specialOwned=curated&&!!state.specialContext
    // Compatibility marker for pre-v5.4.6 static gates: window.SBB_SELECTED_EVENT?.clear?.({reason:'selected curated match has no standard Game Center provider'
  ...
  function enforceCuratedOwnership(reason='curated ownership guard'){
    ...
    try{
      ...
      if(!active?.__sbbCuratedOverride||activeKey!==expectedKey){
        ...
        if(typeof tuneProgramIndexV5==='function')tuneProgramIndexV5(idx,{userInitiated:false,reason:`v5.4.6 ownership repair: ${reason}`});
      }
  ...
  function activateQueue(items,label,startIndex=0){
    ...
    return tuneCuratedIndex(bounded,{userInitiated:true,reason:`v5.4.6 curated programming: ${label}`});
  }
      ...
      const wrapped=function(slot,err,userInitiated=false){
        ...
        if(state.queueActive&&item?.__sbbCuratedOverride&&(!interrupted||specialOwns)){
          ...
          if(alt){
            ...
            return tuneCuratedIndex(idx,{userInitiated:false,reason:'v5.4.6 same-game curated fallback'});
          }
          ...
          if(next>=0)return tuneCuratedIndex(next,{userInitiated:false,reason:'v5.4.6 next special-event highlight after unavailable source'});
