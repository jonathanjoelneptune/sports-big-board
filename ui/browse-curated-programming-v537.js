/* Sports Big Board v5.4.9 — Game Center Score + Transition Bumpers
...
(() => {
  'use strict';
  if(window.SBB_CURATED_BROWSE?.version==='5.4.9') return;

  const VERSION='5.4.9';
  const FAVORITES_KEY='sbb.curation.favorites.v1';
  const ENTITY_CATALOG_KEY='sbb.browse.entity-catalog.v535';
  const ENTITY_CATALOG_TTL_MS=12*60*60*1000;
  // Legacy regression marker: ENTITY_CATALOG_TTL_MS=6*60*60*1000 was the v5.3.5 hard-expiry contract; v5.4.9 replaces it with stale-while-refresh.
  const ENTITY_CATALOG_MAX_STALE_MS=90*24*60*60*1000;
  const CONTROLLER_PREWARM_LEAGUES=Object.freeze(['MLB','NFL','NBA','NHL','EPL','MLS','NCAAF','WC2026','LLWS2026']);
  const ENTITY_CONTEXT_REFRESH_MS=10*60*1000;
  const TEAM_THEME_KEY='sbb.team-theme.enabled.v1';
  // v5.4.9: browser-proven embed health persists independently from release
  ...
  const CURATED_MEDIA_HEALTH_TTL_MS=90*24*60*60*1000;
  ...
  const state={
    ...
    // v5.4.9: Play All is a deliberately conservative unattended queue. A
    ...
    playAllMode:false,playAllStartTimer:null,playAllEpoch:0,
  };
  ...
  function entityCatalogAge(league){return Date.now()-Number(state.entityCatalogSavedAt.get(clean(league).toUpperCase())||0);}
  function entityCatalogFresh(league){return entityCatalogAge(league)<ENTITY_CATALOG_TTL_MS;}
  function entityCatalogUsable(league){return entityCatalogAge(league)<ENTITY_CATALOG_MAX_STALE_MS;}
  ...
  function loadCuratedMediaHealth(){
    try{
      const raw=JSON.parse(localStorage.getItem(CURATED_MEDIA_HEALTH_KEY)||'{}'),now=Date.now(),out={};
      for(const [key,value] of Object.entries(raw&&typeof raw==='object'?raw:{})){
        const at=Number(value?.at||0);if(!key||!at||now-at>CURATED_MEDIA_HEALTH_TTL_MS)continue;
        out[key]={status:value?.status==='success'?'success':'failed',at,successes:Number(value?.successes||0),failures:Number(value?.failures||0),reason:clean(value?.reason).slice(0,120)};
      }
      return out;
    }catch(_){return {};}
  }
  function persistCuratedMediaHealth(){
    try{
      const rows=Object.entries(state.curatedMediaHealth||{}).sort((a,b)=>Number(b[1]?.at||0)-Number(a[1]?.at||0)).slice(0,500);
      localStorage.setItem(CURATED_MEDIA_HEALTH_KEY,JSON.stringify(Object.fromEntries(rows)));
    }catch(_){}
  }
  function mediaHealthKey(item){
    const url=clean(item?.url||item?.mediaUrl||item?.canonicalUrl),yt=clean(item?.youtubeId)||youtubeIdFrom(item?.providerMediaId)||youtubeIdFrom(url);
    return clean(item?.__sbbMediaHealthKey||item?.assetKey||item?.id||yt||url);
  }
  function mediaHealthStatus(item){const key=mediaHealthKey(item);return key?(state.curatedMediaHealth?.[key]?.status||'unknown'):'unknown';}
  function mediaHealthScore(item){const status=mediaHealthStatus(item);return status==='success'?3:(status==='failed'?-3:0);}
  function markCuratedMediaHealth(item,ok,reason=''){
    const key=mediaHealthKey(item);if(!key)return;
    const prior=state.curatedMediaHealth[key]||{};state.curatedMediaHealth[key]={status:ok?'success':'failed',at:Date.now(),successes:Number(prior.successes||0)+(ok?1:0),failures:Number(prior.failures||0)+(ok?0:1),reason:clean(reason).slice(0,120)};
    persistCuratedMediaHealth();
  }
  function mediaUsable(item){const runtime=clean(item?.runtimeState).toUpperCase(),blocked=item?.embedAllowed===false||item?.embeddable===false||['FAILED','BLOCKED','UNPLAYABLE'].includes(runtime);const proven=!!(item&&(item.verified===true||Number(item.verified)===1||runtime==='SUCCESS'||Number(item.runtimeSuccessAt)>0));return !!(proven&&(!blocked||Number(item?.runtimeSuccessAt)>0)&&(item.url||item.youtubeId||item.mediaUrl));}
  function sortedMediaPool(pool=[]){
    return pool.filter(mediaUsable).slice().sort((a,b)=>(mediaHealthScore(b)-mediaHealthScore(a))||(Number(b.runtimeSuccessAt||0)-Number(a.runtimeSuccessAt||0))||(Number(b.verified)-Number(a.verified))||(Number(b.verifiedAt||0)-Number(a.verifiedAt||0)));
  }
  function allMediaForAuditRow(row,maxItems=6){
    const tiers=row?.tiers||{},tierOrder=['gold','green','extended','blue'],tierRank=Object.fromEntries(tierOrder.map((x,i)=>[x,i])),candidates=[],seen=new Set();
    // Rank across tiers rather than exhausting GOLD before considering a
    // browser-proven GREEN/EXTENDED source. Quality remains the tiebreaker, but
    // an asset that actually embeds outranks a higher-tier asset known to fail.
    for(const tier of tierOrder)for(const media of sortedMediaPool(tiers[tier]||[])){
      ...
      if(!key||seen.has(key))continue;seen.add(key);candidates.push({tier,media,key});
    }
    candidates.sort((a,b)=>(mediaHealthScore(b.media)-mediaHealthScore(a.media))||(Number(b.media?.runtimeSuccessAt||0)-Number(a.media?.runtimeSuccessAt||0))||((tierRank[a.tier]??99)-(tierRank[b.tier]??99))||(Number(b.media?.verifiedAt||0)-Number(a.media?.verifiedAt||0)));
    return candidates.slice(0,maxItems).map(({tier,media})=>({tier,media}));
  }
  ...
  function programItemFromAudit(row,media,tier,index=0){
    ...
    return {
      ...
      // Preserve the database's exact verification/readiness evidence on the
      // queue item. The prior build collapsed every verified catalog row down to two
      // booleans, which made already-known clips look cold again to downstream
      // playback/readiness code.
      verifiedPlayable:true,verified:true,verifiedAt:Number(media?.verifiedAt||0)||0,
      runtimeSuccessAt:Number(media?.runtimeSuccessAt||0)||0,runtimeState:clean(media?.runtimeState),
      embedAllowed:media?.embedAllowed!==false,embeddable:media?.embeddable!==false,
      __sbbDatabaseVerified:true,__sbbBrowserProven:mediaHealthStatus(media)==='success'||Number(media?.runtimeSuccessAt||0)>0,
      __sbbCuratedOverride:true,__sbbBrowseV537:true,
  ...
  function installLegacyCfbGuard(){
    ...
    // v5.4.9: the special-event header is display-only. Do not observe/filter its
  ...
  function enterSpecialContext(league,label,{initialAll=true}={}){
    ...
    setTimeout(()=>{primeSpecialEventGames(league);if(initialAll)activateHistorical({all:true});},0);
  ...
  function clearSpecialContext(){if(!state.specialContext)return;const prior=state.specialContext;state.specialContext=null;state.specialEventGames=[];state.specialEventDataEpoch++;restoreCuratedGameCenterFallback();syncSpecialContextUi();window.dispatchEvent(new CustomEvent('sbb:special-context',{detail:{active:false,league:prior.league}}));}
  ...
  async function fetchFullEntityCatalog(league,{forceMetadata=false}={}){
    ...
    // v5.4.9 prefers the backend's persisted participant index. It is built from
    ...
    // Compatibility fallback for a backend that has not completed the v5.4.9
  ...
  async function controllerEntitiesForLeague(league){
    const selected=clean(league).toUpperCase();if(!selected||selected==='ALL')return [];
    const cached=state.entityCatalogCache.get(selected)||[];
    // Stale-while-refresh: a saved team/player directory is immediately usable.
    // Never make controller navigation wait merely because the refresh clock fired.
    if(cached.length&&entityCatalogUsable(selected)){
      if(!entityCatalogFresh(selected)&&!state.entityCatalogInflight.has(selected))setTimeout(()=>primeEntityCatalog({render:false,forceRefresh:true,league:selected}),0);
      return cached.slice();
    }
    if(state.entityCatalogInflight.has(selected)){
      try{const names=await state.entityCatalogInflight.get(selected);if(names?.length)return names.slice();}catch(_){}
    }
    const names=await primeEntityCatalog({render:false,forceRefresh:true,league:selected});
    if(names?.length)return names.slice();
    const fallback=(selected===state.league)?entitiesFromRows(state.specialEventGames.length?state.specialEventGames:state.games):[];
    return [...new Set((fallback.length?fallback:currentEntities(selected)).map(clean).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  }

  async function controllerEntityEntriesForLeague(league){
    const selected=clean(league).toUpperCase();if(!selected||selected==='ALL')return [];
    const names=await controllerEntitiesForLeague(selected);const seen=new Set(),entries=[];
    for(const name of names){
      const display=clean(name),key=norm(display);if(!display||!key||seen.has(key))continue;seen.add(key);
      const meta=entityMetaFor(display,selected)||{};
      entries.push({name:display,abbreviation:clean(meta.abbreviation||meta.abbr),logo:clean(meta.logo||meta.flag||meta.image),country:clean(meta.country||meta.countryCode),kind:clean(meta.kind)});
    }
    return entries.sort((a,b)=>a.name.localeCompare(b.name));
  }
  const sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));
  async function controllerSelectEntityContext(league,{special=false,label=''}={}){
    const selected=clean(league).toUpperCase();if(!selected||selected==='ALL')return false;
    if(special||!isCoreLeague(selected)){
      if(state.specialContext?.league!==selected)enterSpecialContext(selected,clean(label)||leagueLabel(selected),{initialAll:false});
      else{state.league=selected;state.entityType=entityTypeFor(selected);syncSpecialContextUi();syncLeagueUi();}
      await sleep(0);return true;
    }
    if(state.specialContext)clearSpecialContext();
    // Use the same score-filter authority the top Team Browse uses, then wait for
    // its capture/deferred handlers to settle before starting historical lookup.
    try{
      if(selectedLeague()!==selected){
        const btn=[...document.querySelectorAll('#scoreFilters [data-score-filter]')].find(x=>clean(x.dataset.scoreFilter).toUpperCase()===selected);
        btn?.click?.();
      }
    }catch(_){}
    for(let i=0;i<24&&selectedLeague()!==selected;i++)await sleep(25);
    try{scoreRibbonLeagueFilter=selected;}catch(_){}
    state.league=selected;state.entityType=entityTypeFor(selected);syncLeagueUi();
    await sleep(0);return state.league===selected;
  }
  async function controllerBrowseEntity(league,entity,{special=false,label=''}={}){
    const selected=clean(league).toUpperCase(),name=clean(entity);if(!selected||!name)return false;
    await controllerSelectEntityContext(selected,{special,label});
    // Prime metadata without blocking history. The selection itself follows the
    // exact same activateHistorical authority used by a click in top Team Browse.
    primeEntityCatalog({render:false,league:selected}).catch(()=>{});
    // Compatibility marker for v5.4.8 regression: return activateHistorical({entity:name});
    await activateHistorical({entity:name,controller:true});
    return state.mode==='history'&&state.league===selected&&state.entity===name;
  }

  async function primeEntityCatalog({render=true,forceRefresh=false,league:requestedLeague=''}={}){
    const league=clean(requestedLeague||state.league).toUpperCase();if(!league||league==='ALL')return [];
    ...
    if(render&&state.open&&!cached.length&&state.league===league){const host=$('sbbBrowseSuggestions');if(host)host.insertAdjacentHTML('afterbegin',`<div class="sbb-browse-cache-status">Building complete ${state.entityType==='player'?'player':'team'} library once; future opens are instant.</div>`);}
    ...
    try{const names=await promise;if(render&&state.open&&state.league===league)renderSuggestions(names,$('sbbBrowseSearch')?.value||'');return names;}catch(err){
      const fallback=cached.length?cached:(league===state.league?currentEntities(league):[]);if(render&&state.open&&state.league===league)renderSuggestions(fallback,$('sbbBrowseSearch')?.value||'');return fallback;
    }
  }

  async function prewarmControllerEntityCatalogs(){
    const dynamic=[...document.querySelectorAll('#sbbSpecialEventsMenu [data-special-competition]')].map(x=>clean(x.dataset.specialCompetition).toUpperCase()).filter(Boolean);
    const leagues=[...new Set([...CONTROLLER_PREWARM_LEAGUES,...dynamic].filter(x=>x&&x!=='ALL'&&x!=='CFB'))];
    for(const league of leagues){
      try{await primeEntityCatalog({render:false,league});}catch(_){}
      await sleep(90);
    }
  ...
  async function activateHistorical({entity='',all=false,controller=false}={}){
    setOpen(false);
    ...
    try{
      let rows=await fetchAuditRows(state.league,state.entity,MAX_AUDIT_ROWS);let filtered=state.entity?rows.filter(row=>gameHasEntity(row?.game,state.entity)):rows;
      // A controller selection must never dead-end because the backend q index is
      // lagging a participant-directory refresh. If the targeted query returns no
      // usable rows, perform one league-wide audit fallback and filter locally.
      if(state.entity&&!filtered.length){
        const fullRows=await fetchAuditRows(state.league,'',MAX_ENTITY_AUDIT_ROWS);const recovered=fullRows.filter(row=>gameHasEntity(row?.game,state.entity));
        if(recovered.length){rows=fullRows;filtered=recovered;}
      }
  ...
  function releaseCuratedQueue(reason='return to daily programming'){
    ...
    // v5.4.9: curated Team/Player/Special Event queues are user-owned only while
  ...
  function localTodayISO(){const d=new Date(),shifted=new Date(d.getTime()-d.getTimezoneOffset()*60000);return shifted.toISOString().slice(0,10);}
  function playAllTier(item){const tier=clean(item?.tier||item?.historicalTier||item?.mediaTier||item?.programType).toLowerCase();return tier==='purple'?'extended':tier;}
  function playAllVerified(item){
    if(!item||!(item.youtubeId||item.mediaUrl||item.url))return false;
    const runtime=clean(item.runtimeState).toUpperCase();if(item.embedAllowed===false||item.embeddable===false||['FAILED','BLOCKED','UNPLAYABLE'].includes(runtime))return false;
    if(mediaHealthStatus(item)==='failed')return false;
    return item.verifiedPlayable===true||item.verified===true||Number(item.runtimeSuccessAt||0)>0||mediaHealthStatus(item)==='success';
  }
  function playAllCandidateRank(item){
    const health=mediaHealthStatus(item)==='success'||Number(item?.runtimeSuccessAt||0)>0?1000:0;
    const tier=playAllTier(item),quality=tier==='green'?400:(tier==='extended'?360:(tier==='gold'?300:(tier==='blue'?220:100)));
    const transport=item?.youtubeId?40:0,proof=item?.verifiedPlayable===true?30:(item?.verified===true?20:0);
    return health+quality+transport+proof+Math.min(20,Math.floor(Number(item?.verifiedAt||0)/1e12));
  }
  function playAllCandidatePool(game){
    const out=[],seen=new Set();for(const item of [...(game?.items||[]),...(game?.alternates||[])]){const key=clean(item?.id||item?.youtubeId||item?.mediaUrl||item?.url);if(!key||seen.has(key)||!playAllVerified(item))continue;seen.add(key);out.push(item);}
    return out.sort((a,b)=>playAllCandidateRank(b)-playAllCandidateRank(a));
  }
  function playAllGameComplete(game){
    const source=game?.scoreMatch||game?.match||game?.row||{},item=game?.items?.[0]||{};
    if(source?.final===true||source?.completed===true||source?.isFinal===true||game?.final===true||game?.completed===true)return true;
    try{if(typeof isFinal==='function'&&isFinal(game?.match||source))return true;}catch(_){}
    const raw=clean(source?.status||source?.state||source?.statusType||game?.status||item?.status).toLowerCase();
    const status=raw.replace(/[^a-z0-9]+/g,' ').trim();
    if(/\b(postponed|canceled|cancelled|scheduled|pregame|pre game|live|in progress|delayed|suspended)\b/.test(status))return false;
    if(/\b(final|finished|complete|completed|postgame|ended|full time)\b/.test(status)||status==='ft')return true;
    const date=clean(game?.date||source?.date||item?.date).slice(0,10),today=localTodayISO();
    const away=source?.awayScore??game?.row?.awayScore??item?.awayScore,home=source?.homeScore??game?.row?.homeScore??item?.homeScore;
    const hasScore=away!==null&&away!==undefined&&away!==''&&home!==null&&home!==undefined&&home!=='';
    const pool=playAllCandidatePool(game),hasVerifiedRecap=pool.some(x=>['green','extended','gold'].includes(playAllTier(x))&&(x.overview!==false||clean(x.programType).toLowerCase()==='recap'));
    // On a historical date, a verified recap plus no postponed/cancelled status is
    // sufficient completion evidence even if lazy score enrichment has not returned.
    // TODAY is stricter: without an authoritative final/completed flag/status above,
    // Play All skips the game. A live score alone is never completion proof.
    if(date&&date<today)return hasScore||hasVerifiedRecap;
    if(date===today)return false;
    return false;
  }
  function playAllProjectGame(game){const pool=playAllCandidatePool(game);if(!pool.length||!playAllGameComplete(game))return null;return {...game,items:[pool[0]],alternates:pool.slice(1),__sbbPlayAllEligible:true};}
  function playAllEligibleGames(games=state.games){return (games||[]).map(playAllProjectGame).filter(Boolean);}
  function clearPlayAllStartWatchdog(){if(state.playAllStartTimer){clearTimeout(state.playAllStartTimer);state.playAllStartTimer=null;}state.playAllEpoch++;}
  function curatedItemIsPlaying(item){
    if(!item)return false;let active=null;try{active=typeof clip==='function'?clip(currentIndex):null;}catch(_){}if(programKey(active)!==programKey(item))return false;
    try{const snap=window.SBB_PLAYBACK_SESSION?.snapshot?.()||{};if(String(snap.state||'').toLowerCase()==='playing')return true;}catch(_){}
    try{const player=players?.[activeSlot];if(player?.getPlayerState?.()===1||Number(player?.getCurrentTime?.()||0)>.35)return true;}catch(_){}
    try{const native=activeSlot==='A'?$('nativeA'):$('nativeB');if(native&&!native.paused&&Number(native.currentTime||0)>.35)return true;}catch(_){}
    return false;
  }
  function advancePlayAllAfterSlowStart(item,index){
    if(!state.playAllMode||!state.queueActive||state.queueItems[index]!==item||curatedItemIsPlaying(item))return false;
    ...
    if(alt){state.queueItems[index]=alt;try{PROGRAM[index]=alt;GENERAL_PROGRAM[index]=alt;}catch(_){}return tuneCuratedIndex(index,{userInitiated:false,reason:'v5.4.9 Play All fast same-game fallback'});}
    for(let next=index+1;next<state.queueItems.length;next++){const candidate=state.queueItems[next],key=programKey(candidate);if(candidate&&(!key||!state.failedCuratedMedia.has(key)))return tuneCuratedIndex(next,{userInitiated:false,reason:'v5.4.9 Play All skipped slow verified source'});}
  ...
  function armPlayAllStartWatchdog(item,index){
    clearPlayAllStartWatchdog();if(!state.playAllMode||!playAllVerified(item))return;const epoch=state.playAllEpoch,key=programKey(item);
    const browserProven=mediaHealthStatus(item)==='success'||Number(item?.runtimeSuccessAt||0)>0||item?.__sbbBrowserProven===true;
    const timeoutMs=browserProven?5500:7500;
    state.playAllStartTimer=setTimeout(()=>{state.playAllStartTimer=null;if(epoch!==state.playAllEpoch||programKey(state.queueItems[index])!==key)return;advancePlayAllAfterSlowStart(item,index);},timeoutMs);
  }
  ...
  function syncSpecialGameCenterLabels(active){
    for(const id of ['gameCenterTabBtn','gameCenterDrawerBtn']){const btn=$(id);if(btn)btn.textContent=active?'MATCH CENTER':'GAME CENTER';}
  }
  function restoreCuratedGameCenterFallback(){
    // Special Event context itself owns the side panel. A stray non-curated
    // renderQueue/Game Center callback is not allowed to tear this surface down.
    if(state.specialContext)return;
    document.body?.classList.remove('sbb-special-event-match-center');syncSpecialGameCenterLabels(false);
    const panel=$('sbbSpecialMatchCenter');panel?.remove();
  ...
  function ensureSpecialMatchCenter(){
    let panel=$('sbbSpecialMatchCenter');if(panel)return panel;
    const host=$('gameCenterPane')||$('gameCenterContent')?.parentElement||$('infoDrawer');if(!host)return null;
    panel=document.createElement('section');panel.id='sbbSpecialMatchCenter';panel.className='sbb-special-match-center';
    panel.innerHTML='<div class="sbb-special-match-kicker">SPECIAL EVENT MATCH CENTER</div><div class="sbb-special-match-league"></div><div class="sbb-special-matchup"></div><div class="sbb-special-match-score"></div><div class="sbb-special-match-meta"></div>';
    host.appendChild(panel);return panel;
  }
  function showCuratedGameCenterFallback(item){
    const authoritative=item?.__sbbCuratedOverride?item:(state.queueActive?(expectedCuratedItem?.()||item):item);if(!authoritative)return;
    const empty=$('gameCenterEmpty'),content=$('gameCenterContent'),panel=ensureSpecialMatchCenter();
    const away=participantName(authoritative?.awayTeam||authoritative?.away)||clean(authoritative?.awayName||authoritative?.away),home=participantName(authoritative?.homeTeam||authoritative?.home)||clean(authoritative?.homeName||authoritative?.home);
    const matchup=away&&home?`${away} vs ${home}`:clean(authoritative?.queueTitle||authoritative?.title)||'SPECIAL EVENT MATCH';
    const league=leagueLabel(clean(authoritative?.competitionId||authoritative?.league||state.specialContext?.league||state.league).toUpperCase()),date=clean(authoritative?.date).slice(0,10);
    const hasScore=authoritative?.awayScore!==null&&authoritative?.awayScore!==undefined&&authoritative?.homeScore!==null&&authoritative?.homeScore!==undefined;
    const score=hasScore?`${authoritative.awayScore}  –  ${authoritative.homeScore}`:'';const status=clean(authoritative?.status);
    document.body?.classList.add('sbb-special-event-match-center');syncSpecialGameCenterLabels(true);
    if(empty){empty.dataset.sbbCuratedMatch='1';empty.classList.add('hidden');}content?.classList.add('hidden');
    if(panel){
      panel.dataset.sbbEventKey=clean(authoritative?.canonicalEventKey||authoritative?.eventId||programKey(authoritative));
      const q=sel=>panel.querySelector(sel);q('.sbb-special-match-league').textContent=league;q('.sbb-special-matchup').textContent=matchup;q('.sbb-special-match-score').textContent=score;q('.sbb-special-match-score').classList.toggle('hidden',!score);
      q('.sbb-special-match-meta').textContent=[date,status].filter(Boolean).join(' · ')||'Selected highlight';
    }
  }
  ...
  function syncCuratedGameCenterContext(item){
    ...
    // v5.4.9: Special Event navigation is the side-panel authority, even if an
    ...
    // Compatibility marker for pre-v5.4.9 static gates: const specialOwned=curated&&!!state.specialContext
    // Compatibility marker for pre-v5.4.9 static gates: window.SBB_SELECTED_EVENT?.clear?.({reason:'selected curated match has no standard Game Center provider'
    ...
    if(unsupported){
      try{window.SBB_SELECTED_EVENT?.clear?.({reason:'selected Special Event match owns Match Center',source:'browse',force:true});}catch(_){}
      showCuratedGameCenterFallback(authoritative);
  ...
  function enforceCuratedOwnership(reason='curated ownership guard'){
    ...
    try{
      ...
      if(!active?.__sbbCuratedOverride||activeKey!==expectedKey){
        ...
        if(typeof tuneProgramIndexV5==='function')tuneProgramIndexV5(idx,{userInitiated:false,reason:`v5.4.9 ownership repair: ${reason}`});
      }
  ...
  function probeCuratedPlaybackSuccess(item){
    const key=programKey(item);if(!key)return;
    const probe=()=>{
      // Browser playback proof is valuable for every curated queue, not only
      // Special Events. Team/Player Play All previously started a known database
      // clip but never promoted that successful start into browser-health truth, so
      // the next Play All pass could treat the same verified source as cold again.
      if(!state.queueActive||programKey(expectedCuratedItem())!==key)return;
      let playing=false,time=0;
      try{const player=players?.[activeSlot];const code=player?.getPlayerState?.();time=Number(player?.getCurrentTime?.()||0);playing=code===1||time>.6;}catch(_){}
      if(!playing){try{const native=activeSlot==='A'?$('nativeA'):$('nativeB');time=Math.max(time,Number(native?.currentTime||0));playing=!!native&&!native.paused&&time>.6;}catch(_){}}
      if(playing){markCuratedMediaHealth(item,true,'browser-proven embedded playback');if(state.playAllMode&&programKey(expectedCuratedItem())===key)clearPlayAllStartWatchdog();}
    };
    for(const ms of [1200,3000,7000,12000])setTimeout(probe,ms);
  }
  ...
  function tuneCuratedIndex(index,{reason='curated programming',userInitiated=true}={}){
    ...
    probeCuratedPlaybackSuccess(item);armPlayAllStartWatchdog(item,index);
  ...
  function activateQueue(items,label,startIndex=0){
    ...
    return tuneCuratedIndex(bounded,{userInitiated:true,reason:`v5.4.9 curated programming: ${label}`});
  }
  function playGames(games,label,{safePlayAll=false}={}){
    if(safePlayAll){
      clearPlayAllStartWatchdog();games=playAllEligibleGames(games);
      if(!games.length){state.playAllMode=false;try{setFeedNote?.('Play All • no completed games with verified media are ready yet');}catch(_){}return false;}
      state.playAllMode=true;try{setFeedNote?.(`Play All • ${games.length} completed game${games.length===1?'':'s'} • verified media only`);}catch(_){}label=`${label} • verified completed games`;
    }
    primeCuratedAlternates(games);return activateQueue(queueItemsForGames(games),label,0);
  }
  function playFrom(index){const games=state.games.slice(index);clearPlayAllStartWatchdog();state.playAllMode=false;if(!games.length)return false;return playGames(games,state.entity||state.facet||`${leagueLabel(state.league)} highlights`);}
  function playAll(){return playGames(state.games,state.entity||state.facet||`${leagueLabel(state.league)} highlights`,{safePlayAll:true});}
    ...
    try{
      ...
      const wrapped=function(slot,err,userInitiated=false){
        ...
        if(state.queueActive&&item?.__sbbCuratedOverride&&(!interrupted||specialOwns)){
          ...
          const failedKey=programKey(item);if(failedKey)state.failedCuratedMedia.add(failedKey);
          markCuratedMediaHealth(item,false,err?.message||'curated playback failure');
          ...
          if(alt){
            ...
            return tuneCuratedIndex(idx,{userInitiated:false,reason:'v5.4.9 same-game curated fallback'});
          }
          ...
          if(next>=0)return tuneCuratedIndex(next,{userInitiated:false,reason:'v5.4.9 next special-event highlight after unavailable source'});
  ...
  function init(){loadEntityCatalogStore();if(!ensureUi())return;installLegacyCfbGuard();captureScoreRibbonHeight();syncLeagueUi();bind();patchRenderQueue();patchPlaybackFailure();patchYouTubePlayerError();setTimeout(()=>primeEntityCatalog(),0);setTimeout(()=>prewarmControllerEntityCatalogs(),700);setTimeout(()=>prewarmControllerEntityCatalogs(),6500);setTimeout(patchRenderQueue,350);setTimeout(patchPlaybackFailure,500);setTimeout(patchYouTubePlayerError,650);setTimeout(()=>{captureScoreRibbonHeight();hideLegacyCfb();},500);setTimeout(hideLegacyCfb,1800);}
  ...
  window.SBB_CURATED_BROWSE=Object.freeze({
    ...
    controllerEntities:league=>controllerEntitiesForLeague(league),controllerEntityEntries:league=>controllerEntityEntriesForLeague(league),
    controllerBrowseEntity:(league,entity,options={})=>controllerBrowseEntity(league,entity,options),controllerPrimeAll:()=>prewarmControllerEntityCatalogs(),
