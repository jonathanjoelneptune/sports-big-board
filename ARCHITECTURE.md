# Sports Big Board v4.1.16 Architecture

## v4.1.16 EPL pinned playlist resolution boundary

Known 2026-27 Premier League and NBC playlist URLs are direct source identities. `playlistItems.list` is attempted on those pins before any channel catalog lookup. The trusted Premier League and NBC Sports channel IDs are retained only as fallback repair/future-season discovery surfaces. If a pin returns zero items, the channel catalog may resolve an exact normalized playlist title and retry once with the canonical ID. `[EPL PLAYLIST INDEX]` and `[EPL PLAYLIST VIDEOS]` expose each stage. Rule Game Catch-up v5 bumps only the EPL playlist lanes to source v2; Rule Collection Catch-up v4 reopens only EPL Every Goal. NFL/MLS/matcher v7 and unrelated Silver data are not reset.
...
## v4.1.16 EPL playlist acquisition boundary

Rule Game Catch-up v4 preserves strict affinity: `green-gap-1 → NFL`, `green-gap-2 → MLS`, `green-gap-3 → EPL`. Discovery remains v15, so v4.1.16 does not globally invalidate unrelated historical discovery. Rule Collection Catch-up v3 is narrowly reopened for the official EPL Every Goal playlist and does not replay the already-completed MLS/legacy Silver migration.
...
Known 2026-27 Premier League/NBC playlist IDs are bootstrap anchors. The official Premier League channel is enumerated automatically. The trusted NBC season playlist is resolved through `playlists.list(id=...)` to learn the broadcaster channel id, then other Premier League season playlists on that same channel can be enumerated. No EPL playlist lane calls `search.list`.
...
### v4.1.16 normalized historical catalog baseline
...
### v4.1.16 targeted Rule Game Catch-up v3
...
The MLS/EPL Silver replay remains a dedicated post-seed worker, but v4.1.16 makes its accounting idempotent: candidates examined, qualifying, new unique assets, reused assets, new links, duplicate links, and rejects are tracked separately. A persisted Rule Collection Catch-up v2 completion marker means reboot does not restart finished migration work.
...
### v4.1.16 fail-closed event association
...
### v4.1.16 structural preflight boundary
...
## v4.1.16 bounded discovery concurrency
...
## v4.1.16 tier-aware provider staging
...
### Fixed historical seed boundary (v4.1.16)
...
## v4.1.16 official content acquisition adapters
