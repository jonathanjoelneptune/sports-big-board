# Sports Big Board v4.1.11 Architecture
...
## v4.1.11 NFL.com official recap acquisition
...
### v4.1.11 normalized historical catalog baseline
...
`HISTORY_DISCOVERY_VERSION = 14` is fresh discovery bookkeeping on top of the reconstructed v4 catalog.

### v4.1.11 targeted rule catch-up

NFL, MLS, and EPL media-policy changes are migrated with independent source/objective versions instead of a global catalog reset. The official-source queue prioritizes those three leagues newest-first to the fixed historical floor. Quick/Green and Extended/Purple are separate objectives for all three leagues, so satisfying Green cannot close an unmet Extended objective. MLS uses separate Match Snapshot and Match Highlights source ledgers; EPL separates PremierLeague.com Match Highlights from NBC Extended Highlights; NFL bumps both official objective versions.

MLS/EPL Silver is replayed by a dedicated one-time collection worker after the historical seed completes. It samples the historical range weekly, lets the strict Silver classifier resolve actual Matchday/Matchweek identity from source metadata, and persists `rule_collection_catchup_v1_complete` so the replay is not repeated on every restart.
...
### v4.1.11 fail-closed event association
...
### v4.1.11 structural preflight boundary
...
## v4.1.11 bounded discovery concurrency
...
## v4.1.11 tier-aware provider staging
...
### Fixed historical seed boundary (v4.1.11)
...
## v4.1.11 official content acquisition adapters
