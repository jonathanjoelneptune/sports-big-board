...
## SendInput
The v5.4.7 R1 Windows helper remains compatible with v5.5.0 and retains SendInput fullscreen delivery plus command-result acknowledgement.

## v5.5.0 Team Select refinements

- The second league radial places **TEAM SELECT / PLAYER SELECT at 12 o'clock** before TODAY and ALL.
- The primary RT league radial uses **league logos** for MLB, NFL, NBA, NHL, EPL, MLS, and NCAAF.
- Team/player catalogs use a **persistent stale-while-refresh cache**. Saved participants render immediately while metadata refreshes in the background.
- Core league catalogs are **prewarmed after startup**, and the backend persists complete ESPN league directories including teams that do not yet have verified media.
- A cold participant request keeps a visible **LOADING / RETRY** radial instead of closing silently.
- Controller team selection settles the canonical score-filter context before historical lookup and falls back to a league-wide audit scan if the targeted history query is temporarily stale.
