# Sports Big Board v3.0.6

## v3.0.6 Green Coverage + background reliability

v3.0.6 turns **Quick Recap / Green coverage into an explicit database backlog** rather than waiting for ordinary chronological backfill to happen to find a better package. The cloud server now runs two complementary historical workers:

- chronological date backfill, which keeps building the 400-day archive; and
- a dedicated **Green-gap queue**, which queries the persistent database for completed games that still lack a verified Green/Gold package. Blue-only games are attacked first, followed by no-media games and Purple-only games.

The Green-gap worker uses official/native event media, official channel uploads/activity indexes and public candidate lanes on every pass. The scarce YouTube `search.list` rescue is throttled globally and shared by league/date, so it is reserved for stubborn Blue/None gaps instead of being burned indiscriminately. Blue remains usable while the server searches for a better recap.

A real background-worker starvation bug is also fixed. Prior releases treated every API poll as `lastInteractive`; an open browser tab polls status/history frequently enough that the old 15-second idle gate could stay false indefinitely. In v3.0.6 passive status/audit/history reads **do not pause backfill**. Background work yields only to actual media transport, a brief user-action window, or an active foreground historical discovery pass. Keeping Sports Big Board open therefore no longer prevents the cloud catalog from improving.

Green classification is strengthened for authoritative exact-event sources. ESPN event video and MLB game-content packages in the 90–420 second range can be promoted to a full recap when their metadata describes the game result, even when the title is player-led instead of literally saying “game highlights.” This is especially aimed at the Blue-heavy MLB archive.

The Historical Database Audit is now live: it auto-refreshes every 30 seconds, shows whether background search is ACTIVE/PAUSED, displays the last Green-gap attempt, and adds **Quick recap coverage by league** so the meaningful metric is games with Green, not the raw number of Blue clips.

`HISTORY_DISCOVERY_VERSION = 10` performs a non-destructive soft reassessment. Existing scores, Blue/Purple/Green assets, Game Centers, runtime playback truth and the persistent cloud database are retained.
...
v3.0.1 established the server-owned historical event/media catalog used by v3.0.6:
...
- **The old two-face Today/Yesterday toggle has been superseded in v3.0.6 by explicit calendar-day navigation.** The underlying viewer-day date authority and ESPN fallback behavior remain in place.
...
v3.0.6 builds on the stabilized score inventory and Game Center work by adding arbitrary historical date context without coupling ribbon browsing to playback.
...
```bash
cd ~/storage/downloads/sports-big-board-v3.0.6/sports-big-board-v3.0.6
bash VERIFY.sh
...
The v3.0.6 regression suite covers, among other things:
...
## v3.0.6 historical playback reconciliation

v3.0.6 fixes the first integration issues revealed by the canonical v2.9 history catalog:
...
## v3.0.6 audit-status reconciliation
...
## v3.0.6 Historical Database Audit
