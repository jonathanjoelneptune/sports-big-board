/* Sports Big Board v4.7.10 ScoreDateStore.
...
(() => {
  ...
  function normalizeDate(value){
    ...
    // v4.7.10: do not clamp future scheduled dates to today. The registry and
  ...
  function setMatches(date,league,rows){
    ...
    day.matches.set(lg,Array.isArray(rows)?rows.slice():[]);
    day.loadedAt=Date.now();
    notify('matches',date);
    return day.matches.get(lg);
  }
  ...
  window.SBB_SCORE_DATE=Object.freeze({
    ...
    hasLeagueMediaSnapshot,markLoading,isLoading,snapshot,subscribe
  });
})();
