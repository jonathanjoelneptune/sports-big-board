/* Sports Big Board v5.5.0 — Native Windows Controller Bridge transport.
   ...
   the local machine. v5.5.0 also permits a tiny whitelisted command channel for
   controller-only fullscreen shortcuts (F11 app fullscreen and F video fullscreen). */
(() => {
  'use strict';
  if(window.SBB_CONTROLLER_NATIVE_BRIDGE?.version==='5.5.0')return;
  const VERSION='5.5.0';
  const PROTOCOL=1;
