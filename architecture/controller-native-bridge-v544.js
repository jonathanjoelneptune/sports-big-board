/* Sports Big Board v5.4.8 — Native Windows Controller Bridge transport.
   ...
   the local machine. v5.4.8 also permits a tiny whitelisted command channel for
   controller-only fullscreen shortcuts (F11 app fullscreen and F video fullscreen). */
(() => {
  'use strict';
  if(window.SBB_CONTROLLER_NATIVE_BRIDGE?.version==='5.4.8')return;
  const VERSION='5.4.8';
  const PROTOCOL=1;
  ...
  window.SBB_CONTROLLER_NATIVE_BRIDGE=Object.freeze({
    version:VERSION,installed:true,protocol:PROTOCOL,supported,gamepad,connect,reconnect,sendCommand,probeLoopbackPermission,close:closeSocket,snapshot,subscribe,
