/* Sports Big Board v4.1.8 — media scope is independent from recap quality. */
(() => {
  ...
  const scoring=/\b(?:every|all)\s+(?:goal|touchdown)s?\s+(?:from|of)\s+(?:matchweek|mwk|matchday|week)\s*\d{1,2}\b/i;
  ...
  function classify(item,{away='',home='',eventId=''}={}){
    ...
    if(['espn-event-video','mlb-game-content','nfl-event-video','official-nfl-club-site','official-nfl-game-highlights','official-nhl-game-recap','official-nhl-condensed-game','official-mls-match-highlights','official-premierleague-match-highlights','trusted-nbc-epl-extended'].includes(st))return SCOPE.GAME;
    if(item?.gamePk&&src.includes('mlb'))return SCOPE.GAME;
    if(round.test(ttl)&&/every goal|best goals|best saves|things you may have missed/i.test(ttl))return SCOPE.ROUND_LEAGUE;
  ...
  function collectionKind(item){const t=text(item),scope=String(item?.mediaScope||'');if(scoring.test(t))return 'SCORING_ROUNDUP';if(topPlays.test(t)||/top (?:goals?|saves?|hits?) (?:from|of) week\s*\d+/i.test(t)||scope===SCOPE.ROUND_LEAGUE&&/best goals|best saves/i.test(t))return 'TOP_PLAYS';if(scope===SCOPE.WEEK_LEAGUE)return 'WEEKLY_RECAP';if(scope===SCOPE.DAY_LEAGUE)return 'DAILY_RECAP';return 'ROUNDUP';}
