/* Sports Big Board v5.3.19 — non-blocking score-ribbon league-day queue + interrupt preservation.
...
(() => {
  'use strict';
  if(window.SBB_SCORE_INTERRUPT_QUEUE?.version==='5.3.19')return;
    ...
    try{
      ...
      setTimeout(()=>{
        ...
        const pump=()=>{
          ...
          if(elapsed>32)console.warn('[SBB v5.3.19] slow score-date match projection yielded after one match',{league,date,ms:Math.round(elapsed),match:clean(match?.name||match?.title||match?.eventId||match?.id)});
