/* Sports Big Board v5.4.7 — non-blocking score-ribbon league-day queue + interrupt preservation.
...
(() => {
  'use strict';
  if(window.SBB_SCORE_INTERRUPT_QUEUE?.version==='5.4.7')return;
  const VERSION='5.4.7';
    ...
    try{
      ...
      setTimeout(()=>{
        ...
        const pump=()=>{
          ...
          if(elapsed>32)console.warn('[SBB v5.4.7] slow score-date match projection yielded after one match',{league,date,ms:Math.round(elapsed),match:clean(match?.name||match?.title||match?.eventId||match?.id)});
