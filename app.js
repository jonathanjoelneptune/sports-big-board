...
/* Sports Big Board v3.0.4 — canonical event/media architecture foundation; v2.5.30 playback behavior preserved. */
...
function scheduleScoreRibbonDayCarousel(){ /* v3.0.4: deliberate date browsing never auto-rotates. */ }
...
function startSportsBigBoardExperience(){
  ...
  try{ fetch('/api/client-log?event=USER_LAUNCH&v=3.0.4',{cache:'no-store'}).catch(()=>{}); }catch(_){}
}
...
function safeStartLiveData(){
  ...
  try{ fetch('/api/client-log?event=APP_LIVE_START&v=3.0.4',{cache:'no-store'}).catch(()=>{}); }catch(e){}
...
function startAssignedPlayback(slot,item,{userInitiated=false,reason='playback',restart=true,forceReload=false,epoch=null}={}){
  ...
  if(slotMedia[slot]==='youtube'){
    ...
    const launch=p=>{
      try{
        // v3.0.4: start every YouTube tune muted first, then restore audio in the
...
function syncSelectedEvent(eventLike,{reason='playback',source='playback'}={}){
  ...
  // v3.0.4: a score-card selection owns the event. Playback may switch media
  ...
  try{
    ...
    startAssignedPlayback(newActive,item,{userInitiated:false,reason:'automatic A/B promotion',restart:false,epoch})
      .then(()=>waitForFirstPlayback(newActive,{timeoutMs:10000,epoch,onTimeout:()=>{
        ...
        if(transitionRecoveryAttempts<1){
          transitionRecoveryAttempts++;
          console.warn('[SBB v3.0.4] transition timeout; retrying assigned media once');
    ...
    transitionInFlight=false;
    console.error('[SBB v3.0.4] swap failure',err);
...
function onPlayerError(slot, code){
  ...
  if(slot === activeSlot){
    ...
    // Referer / API-client identity it now requires. v3.0.4 explicitly serves
...
async function refreshMlsOfficialVideos(matches){
  ...
  for(const date of dates){
    ...
    }catch(e){ console.warn('[SBB v3.0.4] MLS official-channel discovery failed',e); }
  }
  if(found.length){
...
async function rapidEnrichOtherSport(league, matches, candidates,{force=false}={}){
  ...
  for(const m of missing){
    ...
    try{
      ...
      found.push(...rows);
    }catch(e){ console.warn(`[SBB v3.0.4] ${league} rapid team-video search failed`,e); }
  }
  if(found.length){
...
// ---------------- v3.0.4 arbitrary-date score/media discovery ----------------
...
function scheduleHistoricalDiscoveryPoll(date,generation,delay=700){
  ...
  historicalDiscoveryPollTimer=setTimeout(async()=>{
    ...
    }catch(err){console.warn('[SBB v3.0.4] historical discovery poll failed',date,err);scheduleHistoricalDiscoveryPoll(date,generation,1800);}
  },delay);
}
async function startHistoricalDateDiscovery(date,{force=false}={}){
  ...
  try{
    ...
    console.warn('[SBB v3.0.4] historical discovery start failed',date,err);
...
async function loadScoreDateLeagueMatches(league,date){
  ...
  const timezone=Intl.DateTimeFormat().resolvedOptions().timeZone||'Etc/UTC';
  const utcOffsetMinutes=-new Date().getTimezoneOffset();
  // v3.0.4: historical score inventory is server-owned too. Ribbon hydration and
  ...
  if(String(date).slice(0,10)<localDateISO(0)){
    try{
      ...
      console.warn(`[SBB v3.0.4] ${lg} canonical historical score load failed`,date,err);
  ...
  }catch(err){firstError=err;console.warn(`[SBB v3.0.4] ${lg} score load failed`,date,err);}
...
async function rapidHistoricalGameMedia(match,{force=false}={}){
  ...
  try{
    ...
    return ready;
  }catch(err){console.warn(`[SBB v3.0.4] ${league} historical catalog discovery failed`,err);return [];}
}

async function loadScoreDateLeagueMedia(league,date,matches){
  // v3.0.4: full historical discovery belongs to the localhost server. The
...
async function ensureScoreDateLoaded(date,{force=false}={}){
  ...
  const launchMedia=(league,rows)=>{
    ...
    loadScoreDateLeagueMedia(league,date,rows||[]).then(()=>{
      if(scoreBrowseDate===date) renderHistoricalDateDiagnostics(date,historicalDiscoveryState(date));
    }).catch(err=>console.warn('[SBB v3.0.4] historical media hydrate failed',league,date,err));
  };
  ...
  try{
    ...
    const scorePromises=needed.map(async league=>{
      ...
      // v3.0.4 paints each league as soon as that provider finishes rather than
    ...
    console.warn('[SBB v3.0.4] historical score-date load failed',date,err);
    return [];
  }finally{
...
async function refreshSoccerLeague(league,today,yesterday,allCandidates){
  ...
  }catch(mediaErr){ console.warn(`[SBB v3.0.4] ${league} media discovery`,mediaErr); }
...
async function refreshOtherSports(first=false){
  ...
  await Promise.all(configs.map(async([league,key])=>{
    ...
    try{
      ...
      // v3.0.4 deep-dive: schedule identity is fetched from ESPN directly and
      ...
      const loadLeagueDay=async(date,day)=>{
        ...
        try{
          ...
          espnError=err;
          console.warn(`[SBB v3.0.4] ${league} ESPN ${day} schedule read failed`,err);
        }
        if(!rows.length && apiConfigured){
          try{
            ...
            console.warn(`[SBB v3.0.4] ${league} ${day} Highlightly schedule fallback failed`,err);
      ...
      for(const result of highlightResults){
        if(result.error){ mediaErrors++; console.warn(`[SBB v3.0.4] ${league} highlight refresh failed`,result.error); continue; }
      ...
      console.warn(`[SBB v3.0.4] ${league} Highlightly feed failed`,err);
    ...
    setTimeout(reconcileScoreMediaIndicators,0); mergeLiveProgram(extras,false); allCandidates.push(...extras); }
          ...
          return;
        }catch(fallbackErr){ console.warn(`[SBB v3.0.4] ${league} ESPN fallback failed`,fallbackErr); }
      }
...
async function refreshRapidMlbHighlights(date, force=false){
  ...
  try{
    ...
    console.warn('[SBB v3.0.4] rapid MLB highlight refresh failed',err);
...
async function refreshKeyInformation(first=false,force=false){
  ...
  try{
    ...
    console.warn('[SBB v3.0.4] key information refresh failed',err);
...
async function refreshDailyTopPlays(first=false){
  ...
  }catch(err){ console.warn('[SBB v3.0.4] daily Top Plays refresh failed',err); }
}
...
async function requestAIProgrammingRanks(items){
  ...
  try{
    ...
    aiRankLastAt=Date.now();
    console.warn('[SBB v3.0.4] AI programming rank unavailable; deterministic ranking remains active',err);
  }
...
function mergeLiveProgram(candidates, first){
  ...
  // v3.0.4: once the viewer starts a date-specific session, background live-feed
...
function playGameHighlights(matchId, match, providedItems=null){
  ...
  // v3.0.4: score selection also tells localhost to stage the exact native
