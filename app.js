...
/* Sports Big Board v4.1.8 — canonical event/media architecture foundation; v2.5.30 playback behavior preserved. */
...
let PROGRAM = FORCE_BLUE_TEST ? [] : [
  { id:'oKkhTdPR3DA', league:'NBA', title:'Magic at Pistons — Full Game Highlights', subtitle:'Official NBA highlight package' },
  { id:'t44ztyYZ434', league:'NBA', title:'Nets at Lakers — Full Game Highlights', subtitle:'Official NBA highlight package' },
  { id:'eHDl-gryBJM', league:'NBA', title:'Top 10 Plays of the Night', subtitle:'Official NBA top plays' },
  { id:'iWX1ofpnkeE', league:'NBA', title:"NBA's Nightly Recap", subtitle:'Official NBA nightly recap' },
  { id:'Aqi9zT9WVgk', league:'NBA', title:'Spurs at Thunder — Extended Highlights', subtitle:'Official NBA extended highlights' },
  { id:'GbHxc0oxCFI', league:'NBA', title:'2026 Finals — College Highlights', subtitle:'Sports highlight programming fallback' },
];
...
let activeSlot = 'A';
let currentIndex = Math.floor(Math.random()*Math.max(1,PROGRAM.length));
let standbyIndex = (currentIndex + 1) % Math.max(1,PROGRAM.length);
...
async function setScoreBrowseDate(value,{animate=true,hold=9000,load=true}={}){
  ...
  if(date<today){
    ...
    // Full-day discovery is server-owned so every completed game is searched and
    // persisted, independent of which league tab/card the viewer touches.
    startHistoricalDateDiscovery(date).catch(()=>{});
  }else{
  ...
  if(load) maybeAutoplayRoundupForDate(date);
...
function scheduleScoreRibbonDayCarousel(){ /* v4.1.8: deliberate date browsing never auto-rotates. */ }
function wireScoreDayPager(){
  document.querySelectorAll('.score-day-pager').forEach(pager=>{
    ...
    pager.addEventListener('click',e=>{ const btn=e.target.closest('[data-score-date-step]'); if(btn&&!btn.disabled) stepScoreRibbonDate(Number(btn.dataset.scoreDateStep||0)); });
  });
...
function startSportsBigBoardExperience(){
  ...
  if(PROGRAM.length){
    ...
    if(initialized || playerReady[activeSlot] || isNativeItem(clip(currentIndex)) || isContextItem(clip(currentIndex))){
      // The launch button itself is already the browser-trusted user gesture. Do not
      // start a second readiness/recovery loop here; that v2.6.4.1 experiment could
      // race the normal playback controller and leave the bumper owning the picture.
      // Start the current assigned player once and let provider PLAYING events reveal it.
  ...
  try{ fetch('/api/client-log?event=USER_LAUNCH&v=4.1.8',{cache:'no-store'}).catch(()=>{}); }catch(_){}
}
...
const clip = i => PROGRAM[((i % PROGRAM.length) + PROGRAM.length) % PROGRAM.length];

window.onYouTubeIframeAPIReady = () => {
  bindNativePlayers();
  showBumper(currentIndex, 0, 'STARTING NOW');
  setPlaybackUi('starting');
  createPlayer('A', clip(currentIndex).id, true);
  createPlayer('B', clip(standbyIndex).id, false);
  renderQueue();
  renderMetadata();
  safeStartLiveData();
};
...
function safeStartLiveData(){
  ...
  try{ fetch('/api/client-log?event=APP_LIVE_START&v=4.1.8',{cache:'no-store'}).catch(()=>{}); }catch(e){}
...
function bindNativeVideoElement(slot,v){
  ...
  v.addEventListener('playing',()=>{
    ...
    setPlaybackUi('playing');
    try{ const currentItem=clip(currentIndex); window.SBB_PROVIDER_HEALTH?.success?.(currentItem?.provider||currentItem?.source||'DIRECT_VIDEO'); window.SBB_MEDIA_MANIFEST?.markPlaying?.(knownMatchForMedia(currentItem)||window.SBB_SELECTED_EVENT?.get?.()||currentItem,currentItem); reportHistoricalRuntime(currentItem,'PLAYED'); }catch(_){ }
  ...
  v.addEventListener('pause',()=>{
    if(!current()) return;
    if(slot===activeSlot && !transitionInFlight && document.visibilityState==='visible') manualPauseRequested=true;
  });
...
function configureSlotForItem(slot,item,active=false){
  ...
  if(isContextItem(item)){
    ...
    if(v){
      ...
      v.pause();
      const mediaSrc=nativePlaybackUrl(item);
      if(v.getAttribute('src') !== mediaSrc) v.setAttribute('src', mediaSrc);
...
function startAssignedPlayback(slot,item,{userInitiated=false,reason='playback',restart=true,forceReload=false,epoch=null}={}){
  ...
  if(slotMedia[slot]==='youtube'){
    ...
    const launch=p=>{
      try{
        // v4.1.8: start every YouTube tune muted first, then restore audio in the
...
function playSlot(slot){
  if(!sbbPlaybackAllowed({notify:false})) return;
  setPlaybackUi('starting');
...
function pauseSlot(slot){ adapterForSlot(slot).pause(); }
...
function onState(slot, event){
  ...
  if(slot === activeSlot && state === YT.PlayerState.PLAYING){
    ...
    try{ const currentItem=clip(currentIndex); window.SBB_PROVIDER_HEALTH?.success?.(currentItem?.provider||currentItem?.source||'YOUTUBE'); window.SBB_MEDIA_MANIFEST?.markPlaying?.(knownMatchForMedia(currentItem)||window.SBB_SELECTED_EVENT?.get?.()||currentItem,currentItem); reportHistoricalRuntime(currentItem,'PLAYED'); }catch(_){ }
    videoReady[slot] = true;
...
function syncSelectedEvent(eventLike,{reason='playback',source='playback'}={}){
  ...
  // v4.1.8: a score-card selection owns the event. Playback may switch media
...
function programForScoreDate(date){
  ...
  const out=includeRoundups?roundupMediaForScoreDate(date):[]; const seenGames=new Set();
  for(const match of rows){
...
function playDailyRoundup(date,{userInitiated=true}={}){
  ...
  PROGRAM=programForScoreDate(date);currentIndex=0;standbyIndex=PROGRAM.length>1?1:0;manualPauseRequested=false;visibilityResumeWanted=false;
...
function maybeAutoplayRoundupForDate(date,{force=false}={}){
  date=String(date||scoreBrowseDate).slice(0,10);const key=`${date}:${scoreRibbonLeagueFilter}`;
  if(!force&&ROUNDUP_AUTOPLAYED.has(key))return false;
  if(userPlaybackSession||!roundupMediaForScoreDate(date).length||!sbbPlaybackAllowed({notify:false}))return false;
  ROUNDUP_AUTOPLAYED.add(key);return playDailyRoundup(date,{userInitiated:false});
}
...
const PlaybackController={
  tuneProgramIndex(targetIndex,{userInitiated=false,reason='selection',restart=true}={}){
    ...
    if(!isContextItem(item) && !isTopPlaysItem(item) && (item.gamePk||item.matchId||item.eventId||item.scoreGameKey||item.dateGameKey)){
      const scoreMatch=launchScoreMatchForItem(item);
      syncSelectedEvent(scoreMatch?gameCenterSelectionFromScoreMatch(scoreMatch):item,{reason,source:userInitiated?'direct-tune':'program'});
    }
    ...
    if(previousActive!==targetSlot){
      ...
      // after the new play request is issued. The prepared element keeps the
      // original user gesture and does not pay another load/decode cycle.
    ...
    const start=startAssignedPlayback(targetSlot,item,{userInitiated,reason,restart:hotPrepared?false:restart,epoch});
    start.then(()=>{
      ...
      waitForFirstPlayback(targetSlot,{timeoutMs:12000,userInitiated,selectionToken:token,epoch,onTimeout:()=>{
        ...
        if(transitionRecoveryAttempts<1){
          ...
          setPlaybackUi('buffering');
          startAssignedPlayback(targetSlot,item,{userInitiated,reason:`${reason}: exact-media retry`,restart:true,forceReload:true,epoch})
  ...
  try{
    ...
    startAssignedPlayback(newActive,item,{userInitiated:false,reason:'automatic A/B promotion',restart:false,epoch})
      .then(()=>waitForFirstPlayback(newActive,{timeoutMs:10000,epoch,onTimeout:()=>{
        ...
        if(transitionRecoveryAttempts<1){
          transitionRecoveryAttempts++;
          console.warn('[SBB v4.1.8] transition timeout; retrying assigned media once');
    ...
    transitionInFlight=false;
    console.error('[SBB v4.1.8] swap failure',err);
...
function handlePlaybackFailure(slot, err, userInitiated=false){
  ...
  if(userPlaybackSession?.source==='score'){
    ...
    // Historical green bars must represent something that can actually take over
    // the player. If a cached native URL expired or an iframe fails at runtime,
    // perform one forced exact-game refresh before declaring the recap unavailable.
...
function onPlayerError(slot, code){
  ...
  if(slot === activeSlot){
    ...
    // Referer / API-client identity it now requires. v4.1.8 explicitly serves
...
$('playBtn').onclick = () => {
  ...
  if(slotMedia[activeSlot]==='native'){
    const v=nativeEl(activeSlot); if(!v) return;
    if(!v.paused && !v.ended){ manualPauseRequested=true; v.pause(); }
  ...
  if(state === YT.PlayerState.PLAYING || state === YT.PlayerState.BUFFERING){ manualPauseRequested=true; p.pauseVideo(); }
...
async function refreshMlsOfficialVideos(matches){
  ...
  for(const date of dates){
    ...
    }catch(e){ console.warn('[SBB v4.1.8] MLS official-channel discovery failed',e); }
  }
  if(found.length){
...
async function rapidEnrichOtherSport(league, matches, candidates,{force=false}={}){
  ...
  for(const m of missing){
    ...
    try{
      ...
      found.push(...rows);
    }catch(e){ console.warn(`[SBB v4.1.8] ${league} rapid team-video search failed`,e); }
  }
  if(found.length){
...
// ---------------- v4.1.8 arbitrary-date score/media discovery ----------------
...
function scheduleHistoricalDiscoveryPoll(date,generation,delay=700){
  ...
  historicalDiscoveryPollTimer=setTimeout(async()=>{
    ...
    }catch(err){console.warn('[SBB v4.1.8] historical discovery poll failed',date,err);scheduleHistoricalDiscoveryPoll(date,generation,1800);}
  },delay);
}
async function startHistoricalDateDiscovery(date,{force=false}={}){
  ...
  try{
    ...
    console.warn('[SBB v4.1.8] historical discovery start failed',date,err);
...
async function hydrateScoreDateFromHistory(date){
  try{
    ...
    for(const lg of ENABLED_LIVE_LEAGUES){
      ...
      if(Number(state.scoresSavedAt||0)>0){ storeScoreDateLeague(lg,date,state.scores||[]); any=true; }
...
async function loadScoreDateLeagueMatches(league,date){
  ...
  const timezone=Intl.DateTimeFormat().resolvedOptions().timeZone||'Etc/UTC';
  const utcOffsetMinutes=-new Date().getTimezoneOffset();
  // v4.1.8: historical score inventory is server-owned too. Ribbon hydration and
  ...
  if(String(date).slice(0,10)<localDateISO(0)){
    try{
      ...
      console.warn(`[SBB v4.1.8] ${lg} canonical historical score load failed`,date,err);
  ...
  }catch(err){firstError=err;console.warn(`[SBB v4.1.8] ${lg} score load failed`,date,err);}
...
async function rapidHistoricalGameMedia(match,{force=false}={}){
  ...
  try{
    ...
    return ready;
  }catch(err){console.warn(`[SBB v4.1.8] ${league} historical catalog discovery failed`,err);return [];}
}

async function loadScoreDateLeagueMedia(league,date,matches){
  // v4.1.8: full historical discovery belongs to the localhost server. The
...
async function ensureScoreDateLoaded(date,{force=false}={}){
  ...
  const launchMedia=(league,rows)=>{
    ...
    loadScoreDateLeagueMedia(league,date,rows||[]).then(()=>{
      if(scoreBrowseDate===date) renderHistoricalDateDiagnostics(date,historicalDiscoveryState(date));
    }).catch(err=>console.warn('[SBB v4.1.8] historical media hydrate failed',league,date,err));
  };
  ...
  try{
    ...
    if(!force && date<today){
      const hydrated=await hydrateScoreDateFromHistory(date);
      if(hydrated.any && scoreBrowseDate===date) renderScoresFromMatchesCombined(true);
      for(const league of ENABLED_LIVE_LEAGUES){
        if(leagueLoaded(league)) launchMedia(league,SCORE_DATE_STORE?.matches?.(date,league)||[]);
      }
    }

    const needed=force?[...ENABLED_LIVE_LEAGUES]:ENABLED_LIVE_LEAGUES.filter(lg=>!leagueLoaded(lg));
    const scorePromises=needed.map(async league=>{
      const result=await loadScoreDateLeagueMatches(league,date);
      // v4.1.8 paints each league as soon as that provider finishes rather than
    ...
    console.warn('[SBB v4.1.8] historical score-date load failed',date,err);
    return [];
  }finally{
...
async function refreshSoccerLeague(league,today,yesterday,allCandidates){
  ...
  }catch(mediaErr){ console.warn(`[SBB v4.1.8] ${league} media discovery`,mediaErr); }
...
async function refreshOtherSports(first=false){
  ...
  await Promise.all(configs.map(async([league,key])=>{
    ...
    try{
      ...
      // v4.1.8 deep-dive: schedule identity is fetched from ESPN directly and
      ...
      const loadLeagueDay=async(date,day)=>{
        ...
        try{
          ...
          espnError=err;
          console.warn(`[SBB v4.1.8] ${league} ESPN ${day} schedule read failed`,err);
        }
        if(!rows.length && apiConfigured){
          try{
            ...
            console.warn(`[SBB v4.1.8] ${league} ${day} Highlightly schedule fallback failed`,err);
      ...
      for(const result of highlightResults){
        if(result.error){ mediaErrors++; console.warn(`[SBB v4.1.8] ${league} highlight refresh failed`,result.error); continue; }
      ...
      console.warn(`[SBB v4.1.8] ${league} Highlightly feed failed`,err);
    ...
    setTimeout(reconcileScoreMediaIndicators,0); mergeLiveProgram(extras,false); allCandidates.push(...extras); }
          ...
          return;
        }catch(fallbackErr){ console.warn(`[SBB v4.1.8] ${league} ESPN fallback failed`,fallbackErr); }
      }
...
async function refreshRapidMlbHighlights(date, force=false){
  ...
  try{
    ...
    console.warn('[SBB v4.1.8] rapid MLB highlight refresh failed',err);
...
async function refreshKeyInformation(first=false,force=false){
  ...
  try{
    ...
    console.warn('[SBB v4.1.8] key information refresh failed',err);
...
async function refreshDailyTopPlays(first=false){
  ...
  }catch(err){ console.warn('[SBB v4.1.8] daily Top Plays refresh failed',err); }
}
...
async function requestAIProgrammingRanks(items){
  ...
  try{
    ...
    aiRankLastAt=Date.now();
    console.warn('[SBB v4.1.8] AI programming rank unavailable; deterministic ranking remains active',err);
  }
...
function mergeLiveProgram(candidates, first){
  ...
  const launchKeys=new Set(launchRoundups.map(x=>String(x.youtubeId||x.mediaUrl||x.id||'')));
  ...
  // v4.1.8: once the viewer starts a date-specific session, background live-feed
  ...
  if(playbackDateContext?.date){
    ...
    if(scoped.length){
      const playing=clip(currentIndex); const playingId=String(playing?.id||'');
      const found=playingId?scoped.findIndex(x=>String(x?.id||'')===playingId):-1;
      PROGRAM=scoped;
      currentIndex=found>=0?found:Math.min(Math.max(0,currentIndex),PROGRAM.length-1);
  ...
  // Normal background refresh preserves the currently playing item when it is
  // still in the refreshed program; otherwise start at the first available item.
  PROGRAM=GENERAL_PROGRAM;
  const found=currentId ? PROGRAM.findIndex(x=>x.id===currentId) : -1;
  currentIndex=found>=0 ? found : Math.min(currentIndex,PROGRAM.length-1);
  if(currentIndex<0) currentIndex=0;
  if(isGamePlayed(PROGRAM[currentIndex])){
    const target=nextUnplayedIndex(PROGRAM,currentIndex-1,1);
    if(target>=0) currentIndex=target;
  }
  const next=nextUnplayedIndex(PROGRAM,currentIndex,1);
  if(next>=0) prepareStandby(otherSlot(activeSlot), next);
  ensureActiveSlotMatchesCurrent({autoplay:true});
  renderMetadata();
  renderQueue();
}
...
function wireScoreFilters(){
  ...
  if(host&&!host.dataset.wired){
    ...
    host.addEventListener('click',e=>{
      ...
      renderActiveSportKeyInformation();
      loadRoundupsForDate(scoreBrowseDate).then(()=>{renderScoresFromMatchesCombined(false);maybeAutoplayRoundupForDate(scoreBrowseDate,{force:true});});
    });
  }
  wireScoreDayPager();
}
...
function scoreCardPlayableItems(match){
  ...
  if(date){
    pools.push(...scoreMediaForDate(date).filter(x=>{
      ...
      return (!xl||xl===lg) && mediaMatchesScoreGame(x,match) && (sameGameProgramItem(x,match) || scoreRibbonStableGameKey(x)===scoreRibbonStableGameKey(match));
    }));
  }
  ...
  let discovered=[...new Map(
    pools.flat().filter(x=>(!window.SBB_MEDIA_SCOPE||window.SBB_MEDIA_SCOPE.isGame(x,scopeCtx))&&mediaMatchesScoreGame(x,match))
      .map(x=>[String(x.id||x.youtubeId||x.mediaUrl||x.externalUrl),x])
  ).values()];
  try{ window.SBB_MEDIA_MANIFEST?.ingest?.(match,discovered); }catch(_){ }
  let items=(window.SBB_MEDIA_MANIFEST?.playable?.(match)||discovered).filter(runtimeMediaUsable);
...
function buildSilverRoundupScoreCard(date){
  ...
  const line1=document.createElement('div');line1.className='roundup-card-line';line1.textContent=available?(String(items[0]?.collectionKind||'DAILY_RECAP').includes('TOP')?'TOP PLAYS':'DAILY RECAP'):'DAILY RECAP';
...
function playGameHighlights(matchId, match, providedItems=null){
  ...
  // v4.1.8: score selection also tells localhost to stage the exact native
