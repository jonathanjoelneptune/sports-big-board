# Sports Big Board v5.4.0 Controller Readiness Region Map

v5.4.0 is an interaction-architecture release. It does **not** bind the Gamepad API yet. It establishes the semantic focus graph that v5.4.1+ Controller Mode will consume.
...
v5.4.0 only detects pointer and keyboard. It intentionally does **not** call `navigator.getGamepads`, listen for `gamepadconnected`, or poll a controller. v5.4.1 can claim `controller` ownership immediately when meaningful gamepad input is detected.
