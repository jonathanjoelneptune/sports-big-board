/* Sports Big Board v4.6.2 — canonical sport/event/media domain model.
   Provider adapters normalize into SPORT → COMPETITION → EVENT → MEDIA_PACKAGE → MEDIA_ASSET → MOMENT.
   v4.6.2 adds a custom-competition overlay without changing the built-in provider contracts. */
(() => {
  ...
  const COMPETITIONS = Object.freeze({
    ...
    CFB:{id:'CFB',sportId:'american-football',name:'College Football 2026',shortName:'CFB',enabled:true,scoreProvider:'cfb-ranked',mediaProviders:['operator-playlist','youtube'],gameCenterProvider:'competition-builder',seasonId:'CFB2026',seasonYear:2026,startDate:'2026-08-29',endDate:'2027-01-25',eventIcon:'🏈',selectionPolicy:'AP_TOP_25_EITHER_PARTICIPANT',rankingSnapshotPolicy:'IMMUTABLE_WEEKLY'},
  ...
  function enabledCompetitions(){
    const built=Object.values(COMPETITIONS).filter(x=>x.enabled&&!x.synthetic);
    const extra=Object.values(custom()).filter(x=>x.enabled&&x.mainRow);
    return [...built,...extra];
  }
  ...
  function participant(input={},side=''){
    ...
    return {...input,id:clean(input.id??input.teamId??input.clubId),name:clean(input.name??input.teamName??input.displayName),abbreviation:clean(input.abbreviation??input.abbr??input.shortName),side:clean(input.side||side)};
  }
  function event(input={}, competitionId=''){
    ...
    return {...input,entityType:TYPES.EVENT,sportId:clean(input.sportId||input.sport||c.sportId),competitionId:c.id,competitionName:clean(input.competitionName||input.league?.name||c.name),eventKind:clean(input.eventKind||SPORTS[c.sportId]?.eventKind||'event'),eventId:clean(input.eventId)||clean(input.matchId)||clean(input.gamePk)||clean(input.id),scheduledAt:clean(input.scheduledAt||input.date||input.gameDate),status:clean(input.status?.type?.name||input.status?.abstractGameState||input.status?.description||input.status||input.state?.report||input.state?.description||input.state?.status||input.state),participants};
  }
  function mediaAsset(input={}, competitionId=''){
    ...
    return {...input,entityType:TYPES.MEDIA_ASSET,sportId:clean(input.sportId||input.sport||c.sportId),competitionId:c.id,competitionName:clean(input.competitionName||c.name),eventId:clean(input.eventId)||clean(input.matchId)||clean(input.gamePk),mediaId:clean(input.mediaId??input.highlightlyId??input.youtubeId??input.id??input.mediaUrl??input.externalUrl),provider,transport:clean(input.transport||window.SBB_PLAYBACK_TRANSPORTS?.transportForAsset?.(input)),durationSeconds:Number(input.durationSeconds??input.duration??0)||0,sourceQuality:Number(input.sourceQuality??0)||0,verifiedPlayable:input.verifiedPlayable===true,externalOnly:input.externalOnly===true,runtimeState:clean(input.runtimeState||'unknown')};
  }
  const media=mediaAsset;
  function mediaPackage(input={}, parent={}){return {...input,entityType:TYPES.MEDIA_PACKAGE,sportId:clean(input.sportId||parent.sportId),competitionId:upper(input.competitionId||parent.competitionId),eventId:clean(input.eventId||parent.eventId),packageId:clean(input.packageId||input.id),packageType:clean(input.packageType||input.tier||input.mediaKind),assets:Array.isArray(input.assets)?input.assets.map(x=>mediaAsset(x,input.competitionId||parent.competitionId)):[]};}
  function moment(input={}, parent={}){return {...input,entityType:TYPES.MOMENT,sportId:clean(input.sportId||parent.sportId),competitionId:upper(input.competitionId||parent.competitionId),eventId:clean(input.eventId||parent.eventId),momentId:clean(input.momentId??input.id),mediaAssetId:clean(input.mediaAssetId||input.mediaId),description:clean(input.description||input.title),period:input.period??input.inning??null,clock:clean(input.clock),scoreHome:input.scoreHome??null,scoreAway:input.scoreAway??null};}
  function statSection(input={},eventLike={}){const section={...input,title:clean(input.title),columns:Array.isArray(input.columns)?input.columns.map(clean):[],rows:Array.isArray(input.rows)?input.rows:[],teamSide:clean(input.teamSide).toLowerCase(),teamId:clean(input.teamId),teamName:clean(input.teamName),teamAbbreviation:clean(input.teamAbbreviation),category:clean(input.category)};return window.SBB_GAME_CENTER_POLICY?.section?.(section,eventLike)||section;}
  function gameCenter(input={}, parent={}){
    const ev=event(input.event||parent,parent.competitionId||input.competitionId);
    const normalized={entityType:TYPES.GAME_CENTER,version:'1.0',competitionId:upper(input.competitionId||ev.competitionId||parent.competitionId),eventId:clean(input.eventId||ev.eventId||parent.eventId),event:ev,scoreboard:input.scoreboard||{},teamStats:Array.isArray(input.teamStats)?input.teamStats:[],playerStatSections:Array.isArray(input.playerStatSections)?input.playerStatSections.map(x=>statSection(x,ev)):[],timeline:Array.isArray(input.timeline)?input.timeline:[],scoringPlays:Array.isArray(input.scoringPlays)?input.scoringPlays:[],coverage:input.coverage||{},quality:input.quality||{},partial:input.partial===true||input.coverage?.complete===false,updatedAt:input.updatedAt||null,live:!!input.live,source:clean(input.source)};
  ...
  window.SBB_CORE=Object.freeze({version:'4.7.15',TYPES,SPORTS,COMPETITIONS,competition,enabledCompetitions,participant,event,media,mediaAsset,mediaPackage,moment,statSection,gameCenter,editorialPackage,playable});
})();
