...
# Sports Big Board v4.2.2 private soundtrack uploader.
...
# Public Access Prevention is fully compatible with v4.2.2 and should remain
...
if gcloud iam service-accounts add-iam-policy-binding "$VM_SERVICE_ACCOUNT" --project "$PROJECT_ID" \
  ...
  echo "WARNING: Could not grant signBlob to the VM service account; v4.2.2 will use private proxy fallback." >&2
fi
...
echo "v4.2.2 browser transport: backend -> signed GCS redirect -> private proxy fallback"
